...
\addlinespace
$\times$ & Bounded, local padding & \guidance{Lemma~\ref{lemma:pad_productive}; bound open (Problem~\ref{prob:padding_passes})} & Table~\ref{tab:padding_interface_L4} \\
\addlinespace
...
\todo{\guidance{$\lmin$ and $\lmax$ of the leaf star supersede the $\sourcelevel_\coarseIdxtwo$, $\targetlevel_\coarseIdxtwo$ and $\mu_\coarseIdxtwo$ notation still used in Sections~\ref{subsec:refine} and~\ref{subsec:padding}. Fold the latter into the former throughout before submission.}}
...
\end{equation}
and $\domseq^0 = \domseq^{\mathrm{admissible}}$. Note the additional $-2$ in the target level $\targetlevel_{\coarseIdxtwo_p} \! - \! 2$ handed to the quadrisection operator. It ensures that $\vertex{\birthLevel_{\coarseIdxtwo_p}}{\coarseIdxtwo_p}$ has a \guidance{level span} of $2$ or less in $\domseq^p$.

The implementation lifts a vertex's coarsest leaf-star faces by one level and re-tests the \guidance{level span}, iterating until it is at most $2$. That loop is precisely the \guidance{iteration of Definition~\ref{def:graded_vertex_quadrisection}}: its round at level $\l$ is the factor $\quadriVat{\l}$, and it stops where the composition stops, at $\targetlevel_{\coarseIdxtwo_p} \! - \! 2$. \guidance{The implementation re-runs the admissibility padding after each grading round, whereas~\eqref{eq:grading_subdiv_partition} grades every marked vertex first. Both are passes of the loop of Section~\ref{subsec:termination}. That loop is also why the grading is iterated rather than applied once. Being a vertex quadrisection, it can create pinches and inadmissible faces, and lifting the coarse end of one leaf star raises $\targetlevel$ at the neighbouring vertices and can open new grading sites. Neither effect is repaired within its own pass.}
...
\guidance{The three substeps compose into the map
\begin{equation}
...
\end{equation}
which Section~\ref{subsec:termination} iterates to a fixed point.}

\begin{remark}[Padding order matters]
\label{rem:padding_order}
The three padding substeps do not commute: resolving a pinch vertex can lead to an admissible face becoming inadmissible (e.g. Figure~\ref{fig:padding_chain}), which is why $\textsc{Pad}$ always runs pinch resolution before inadmissible-face padding, before grading. \guidance{The interaction runs in every direction. The admissibility padding can in turn create a pinch, and the grading, being a vertex quadrisection, can create both. No ordering of the substeps within a single pass absorbs this, and a single pass never suffices in general.}
\end{remark}

\FloatBarrier

\subsection{\guidance{Termination}}
\label{subsec:termination}

\guidance{Our implementation wraps $\textsc{Pad}$ in a loop that stops once a pass no longer
refines any face, and a counter records the number $q$ of \emph{productive} passes,
\begin{equation}
    \label{eq:pad_iteration}
    \domseq^{(0)} = \domseq^{\mathrm{refine}}, \qquad
    \domseq^{(j+1)} = \textsc{Pad} \big( \domseq^{(j)} \big), \qquad
    \domseq^{\mathrm{pad}} = \domseq^{(q)}.
\end{equation}
That the loop stops at all, and stops at an admissible domain sequence, rests on a single
observation.

\begin{lemma}[A firing predicate refines]
...
\end{lemma}
\begin{proof}
    At a pinch vertex $\vertex{\l}{\coarseIdxtwo}$, two distinct refinement arcs at level $\l$
    are separated in the fan by at least one face outside
    $\refFan{\l}(\vertex{\l}{\coarseIdxtwo})$, since otherwise the fan would be the whole ring
    and hence a single arc. That face is not refined beyond level $\l$, so its children
    incident to $\vertex{\l}{\coarseIdxtwo}$ are not leaf faces, and the last factor
    $\quadriVat{\l}$ of $\quadriVup{\l+1}$ quadrisects at least one of them. The same argument
    covers a boundary pinch, whose unanchored arc is separated from both extremes of the linear
    fan by such a face. An inadmissible face is detected on the leaf faces only,
    Equation~\eqref{eq:leaf_faces}, so $\quadriF$ is not the identity on it. A vertex of level
    span greater than $2$ carries leaf faces below $\targetlevel_\coarseIdxtwo \! - \! 2$, so
    $\quadriVup{\targetlevel_\coarseIdxtwo - 2}$ quadrisects at least one of them. Pinches are
    detected only at levels $\l < \L$ and the grading target never exceeds $\L \! - \! 2$, so
    no repair refines beyond level $\L$.
\end{proof}
...
$\textsc{Pad}$ are exactly the admissible domain sequences of level span at most $2$, and
$\domseq^{\mathrm{pad}} \in \allpartitions$.

This settles the qualitative question and nothing else. The bound it yields is exponential in
$\L$ and permits the very behaviour that adaptivity is meant to avoid. What is wanted instead
is a bound in terms of the control mesh $\T_\lzero$ alone, and the obstruction to one is a
mechanism rather than an accident.

What drives $q$ up is a pinch that outlives the pass which resolves it, and whether one does
is decided by the gaps separating its arcs. A gap of a single face has no refinement-exterior
vertex left, so that face is inadmissible by Definition~\ref{def:admissible_face} and the
admissibility padding of the same pass refines it, merging the two arcs into one. A gap of two
faces survives, and with it the pinch. An interior pinch that outlives its own pass therefore
needs two arcs separated by two gaps of at least two faces each. At valence six the fan has
room for nothing else, $1 \! + \! 2 \! + \! 1 \! + \! 2 = 6$, so both arcs consist of a single
face. Marking vertex stars on one level cannot produce a
single-face arc, because marking a vertex fills its fan, while marking a vertex of its link
refines both faces of the shared edge. On a single level the configuration is thus confined to
the irregular vertices, and Figure~\ref{fig:padding_chain} builds on them.

The figure places valence-$8$ vertices in a row at mesh distance one and marks vertex stars of
the uniform domain sequence, an input the loop can produce by applying $\textsc{Refine}$ to an
admissible domain sequence. Each pass resolves one vertex of the row and pads a single face,
and it is that padded face which opens the same two-arc configuration at the next vertex
along. The repair travels down the row one vertex per pass, so $q$ grows with the length of
the row. How long the row can be made is a property of the control mesh, and no constant
therefore serves for all control meshes.

The restriction to irregular vertices does not survive the loop either. Once $\L \geq 2$ a
marked vertex class may be born below the level at which the fan is read, and its leaf star
can then meet the coarse level in a single face. Refining on the rim of an existing patch
lifts that face and opens a single-face arc at a regular vertex, the mechanism visible in the
right-hand pair of Figure~\ref{fig:refine_interface}. Regular vertices pinch as well, and the
valence count above bounds nothing on its own.

\begin{problem}[Bounding the number of padding passes]
    \label{prob:padding_passes}
    Bound the number $q$ of productive $\textsc{Pad}$ passes by a quantity depending only on
    the control mesh $\T_\lzero$, for instance on the number of its irregular vertices and
    their mutual distances, rather than on $\lvert \F_\L(\T_\L) \rvert$. Confine the faces
    refined by $\textsc{Pad}$ to a neighbourhood of the defects of $\domseq[\L]$ of bounded
    size. The second requirement rules out the degenerate answer
    $\textsc{Pad}(\domseq[\L]) = \T_\L$, which stabilises in one pass and is useless.
\end{problem}

We leave Problem~\ref{prob:padding_passes} open. The two mechanisms above say what a
$\textsc{Pad}$ designed to solve it would have to break, namely the stability of a two-face
gap under the admissibility padding and the single-face arc that rim refinement opens at a
regular vertex. Neither is what the adaptive loop meets in practice. The configurations above
are searched for deliberately, whereas the runs of Section~\ref{subsec:vector_laplace}
stabilise after a single productive pass at every step and pad a small fraction of the faces
refined by the marking, see Table~\ref{tab:padding_interface_L4}.}
...
\begin{figure}[tb]
    ...
    \chainpanel{images/padding_chain/chain_input.pdf}
        {$\domseq^{\mathrm{refine}}$ with four marked vertex stars, pinching $\vertex{}{0}$}
        {fig:padding_chain_input}%
    \hfill%
    \chainpanel{images/padding_chain/chain_pass0.pdf}
        {Pass~1 resolves $\vertex{}{0}$ and its padded face pinches $\vertex{}{1}$}
        {fig:padding_chain_pass1}%
    \hfill%
    \chainpanel{images/padding_chain/chain_pass1.pdf}
        {Pass~2 resolves $\vertex{}{1}$ and its padded face pinches $\vertex{}{2}$}
        {fig:padding_chain_pass2}%
    \hfill%
    \chainpanel{images/padding_chain/chain_pass2.pdf}
        {Pass~3 resolves $\vertex{}{2}$ and reaches the fixed point}
    ...
    a pass is ringed. \guidance{The domain sequence is at $\L = 1$, where the grading predicate
    cannot fire.} Each pass resolves exactly one vertex, whose refinement fan
    $\mathtt{ruuruuur}$ carries two arcs of two and one face separated by gaps of two and three
    and is homogeneous immediately afterwards, and pads a single face. \guidance{The single-face
    arc is not produced by the marking, which contributes runs of two, but is the face padded by
    the preceding pass.} It is that padded face

...
\begin{definition}[Leaf faces]
    ...
    Let $\domseq[\L]$ be a domain sequence of $\T_\L$. Its \emph{leaf faces} $\leafF{\l}$ are all faces on level $\l$ that are not refined any further, i.e.
    \begin{equation}
        \label{eq:leaf_faces}
...
The leaf faces provide a way to represent the domain sequence in a plot, see for example Figure~\ref{fig:active_dofs}. Only the leaf faces are plotted, with the understanding that their coarse parents are also in the domain sequence.

\smallskip

Now that regions and their geometry are settled, we are moving to defining active DoFs on them. \cite{companion_paper} distinguishes between two notions: the DoFs a region naturally carries, and the ones that are \emph{interior} to it in the sense that nothing outside the region touches them.
...
\end{tikzpicture}
\end{center}
\caption{Active $k$-form DoFs for the domain sequence on the left, represented through its leaf faces. The grey shading indicates the level of the leaf faces. The orange and green outlines show the regions $\M_1$ and $\M_2$, respectively, and their ancestors on the previous level. Columns correspond to the level $\l \in \{0, 1, 2\}$, rows to the form degree $k$. Each panel shows the entire uniform mesh $\T_\l$ of its level together with the DoFs of that level. Definition~\ref{def:active_k_form_dofs} selects the DoFs level by level. The blue DoFs indicate the active DoFs at the end of the selection, while the red ones were removed because the region carrying them was refined further.}
...
\begin{example}
    ...
    At the finest level $\l = 2$ nothing is refined further, so $\ancestorPatch{\M_3} =
...
% -------------------------------------------------------------------------
% The covering-star panels that used to stand here have moved down to
% Section~\ref{subsec:operations_on_coverings}, next to their only citation and
% below Equation~\eqref{eq:vertex_star_in_C}, which is where the covering star
% is defined. The other two panels (covering-interior vertices, inadmissible faces) were cut.

% =====================================================================
% The admissibility figure for this section, replacing the old fig:visualisations.
% Panels (a), (b) and (d) are one domain sequence, v6_boundary_threenotches_lvl1,
% read at level 0; panel (c) keeps the nested-centre sequence and is also cited
% from Sections~\ref{subsec:operations_on_coverings} and~\ref{subsec:exactness_atomic}.
% Generated by plot_admissibility.py in the subdiv_FE repo from
% input_partition.vtk + pinch_arcs.txt.
% =====================================================================
\begin{figure}[tb]
    \centering
    \cvsubfig
        {images/visualisations/v6_boundary_threenotches_lvl1_lvl0_partition.pdf}
        {The level-$\lzero$ faces split into leaf faces and the ancestor patch $\ancestorPatch{\M_1}$ (purple)}
        {fig:adm_partition}
    \hfill
    \cvsubfig
        {images/visualisations/v6_boundary_threenotches_lvl1_lvl0_refinement_fan.pdf}
        {The refinement fan (purple) inside the incident faces (outlined) of one vertex}
        {fig:adm_fan}
    \hfill
    \cvsubfig[0.2613]
        {images/visualisations/v6_nested_center_lvl3_exterior_levels.pdf}
        {Vertices coloured by the level from which they are refinement-exterior}
        {fig:adm_exterior}
    \hfill
    \cvsubfig
        {images/visualisations/v6_boundary_threenotches_lvl1_lvl0_admissible_faces.pdf}
        {Leaf faces admissible (green) and inadmissible (orange), with exterior vertices hollow}
        {fig:adm_faces}
    \caption{\guidance{The definition chain of this section read off a single domain sequence. Panel~\subref{fig:adm_partition} shows the only input the predicates use, the splitting of the level-$\lzero$ faces of $\domseq$ into the leaf faces $\leafF{\lzero}(\domseq)$ and the ancestor patch $\ancestorPatch{\M_1}$ of the faces refined further. Panel~\subref{fig:adm_fan} reads that splitting at one vertex: its incident faces $\FVl{\lzero}$ are outlined, and the one of them lying in $\ancestorPatch{\M_1}$ is its refinement fan $\refFan{\lzero}$ (Definition~\ref{def:refinement_fan}), which is why the predicates are level-local. Panel~\subref{fig:adm_exterior} marks every vertex by the level from which its fan is empty, i.e.\ from which it is refinement-exterior (Definition~\ref{def:refinement_exterior_vertex}). Panel~\subref{fig:adm_faces} then runs Definition~\ref{def:admissible_face}: a leaf face is admissible exactly when at least one of its vertices is refinement-exterior, drawn hollow, and the three faces that have no such vertex are inadmissible. Purple keeps the meaning it has in Figure~\ref{fig:pinches} throughout, namely refined beyond the level being read, so the verdict colours are deliberately a different hue.}}
    \label{fig:admissibility_chain}
\end{figure}

We are now ready to define the first geometric predicate of face admissibility.

Both predicates are stated on the \emph{refinement fan} that will be introduced next, following~\cite{companion_paper}. This refinement fan contains all faces of the mesh at a vertex that the sequence refines beyond the current level.
...
\guidance{An extreme case is a domain sequence in which a whole band of coarse faces is enclosed by refinement on both sides, so that every one of its level-$\lzero$ faces is inadmissible while the level-$1$ faces are all admissible.} As we will see in Section~\ref{subsec:exactness_atomic}, the adaptive subdivision $k$-form spaces on \guidance{such a} domain sequence have some interesting properties: There are $3$ linearly dependent active vertex DoFs, $18$ linearly dependent active edge DoFs and $15$ linearly dependent active face DoFs. At the same time, the spaces on this domain sequence have the correct cohomology of $\mathrm{dim}(\mathcal{H}^0) = 1$ and $\mathrm{dim}(\mathcal{H}^1) = \mathrm{dim}(\mathcal{H}^2) = 0$.
...
\guidance{Note that the set of refinement-exterior vertices of $\domseq$ at a given level does not only contain the interior vertices of the patches of $\domseq$ but can also contain interface vertices and vertices on the boundary $\partial \domseq$. For example, Figure~\ref{fig:adm_exterior} displays a domain sequence with nested refinement up to level $3$ towards its centre, with every vertex coloured by the level from which it is refinement-exterior. The outermost layer is refinement-exterior already at level $0$, because none of its incident faces is refined beyond that level, so its fan (Definition~\ref{def:refinement_fan}) is empty there. The next layer becomes refinement-exterior only at level $1$, since at level $0$ it still has incident faces that are refined further. This logic applies to all layers in the figure until we reach the finest level $3$, at which the remaining vertices become refinement-exterior.}

\smallskip

The remainder of this section introduces operators that enable the kinds of refinement that we will later need for the adaptive loop in Section~\ref{subsec:adaptive_loop}. The \emph{face quadrisection operator} $\quadriF(\domseq, \simplexsubset{2}{\l})$ refines a \emph{single-level} set of faces $\simplexsubset{2}{\l} \subseteq \F(\domseq_\l)$ to level $\l+1$ by replacing each face with its four children, that is, it removes the faces $\simplexsubset{2}{\l}$ from level $\l$ and adds their children on level $\l+1$, leaving all other levels untouched.
...
\begin{definition}[Level span of a vertex]
    ...
    incident face, and call $\mu_\coarseIdxtwo \coloneq \targetlevel_\coarseIdxtwo - \sourcelevel_\coarseIdxtwo$ the \emph{level gap} of $\vertex{}{\coarseIdxtwo}$. We write $\mathrm{V}_{\domseq}$ for the set of vertices with at least one active incident face, that is, those for which~\eqref{eq:level_span} is defined, and
    \begin{equation}
...
\robert{
The quadrisection operators enable consistent refinement of the adaptive subdivision $k$-form spaces $\hierSDFspace{k}{\domseq}{\L}$ in the sense that
\begin{equation}
    \label{eq:quadrisection_monotone}
    \hierSDFspace{k}{\domseq}{\L} \subseteq \guidance{\hierSDFspace{k}{\quadriF(\domseq[\L], \simplexsubset{2}{\l})}{}} \qquad \text{and} \qquad \hierSDFspace{k}{\domseq}{\L} \subseteq \guidance{\hierSDFspace{k}{\quadriV{\targetlevel}(\domseq[\L], \vertex{}{\coarseIdxtwo})}{}}.
\end{equation}
...
% --- Figure knobs: covering star and its quadrisection. Moved here from
% Section~\ref{subsec:subdiv_partition}: this is the only place these panels are
% cited, and unlike there, the covering star has by now been defined, by
% Equation~\eqref{eq:vertex_star_in_C} above. Reuses \cvsubfig, still defined in
% the earlier section. -------------------------------------------------------
\begin{figure}[tb]
    \centering
    \cvsubfig[0.32]
        {images/visualisations/neighbour-neighbour_input_with_covering_star.pdf}
        {The active faces of $\domseq$ incident to a vertex $\vertex{}{\coarseIdxtwo}$, carried at mixed levels}
        {fig:covering_star}
    \hfill
    \cvsubfig[0.32]
        {images/visualisations/neighbour-neighbour_refined_with_covering_star.pdf}
        {The star from~\subref{fig:covering_star} after applying $\quadriV{\targetlevel_\coarseIdxtwo + 1}(\domseq, \vertex{}{\coarseIdxtwo})$}
        {fig:covering_star_refined}
    \caption{\guidance{The covering star~\eqref{eq:vertex_star_in_C} of a vertex $\vertex{}{\coarseIdxtwo}$ sitting on a level interface, and the effect of the graded vertex quadrisection operator on it. Unlike the refinement fan of Definition~\ref{def:refinement_fan}, which is read at a single level, the covering star collects the active incident faces at whatever levels they are carried, which is why it spans the interface in~\subref{fig:covering_star}.}}
    \label{fig:covering_star_fig}
\end{figure}

Walking each star position to its covering face at every step is what drags the coarse faces up through \emph{every} intermediate level. The two operators are distinguished by their superscript: $\quadriF$ refines a set of faces flatly from level $\l$ to $\l\!+\!1$, while $\quadriV{\targetlevel}$ homogenises a vertex star towards a target level, and thus only the graded vertex quadrisection operator carries such a level. Figure~\ref{fig:covering_star_refined} displays the result of applying $\quadriV{\targetlevel_\coarseIdxtwo + 1}(\domseq, \vertex{}{\coarseIdxtwo})$ to the star of Figure~\ref{fig:covering_star}: the neighbourhood is first brought up to the finest level $\targetlevel_\coarseIdxtwo$ that $\vertex{}{\coarseIdxtwo}$ already carries, and then one level further.
...
\begin{enumerate}[topsep=3pt, itemsep=-2pt]
    ...
    \begin{equation}
        \domseq^{p} = \quadriV{\tilde{\targetlevel}_{\coarseIdxtwo_p}} \big( \domseq^{p-1}, \, \vertex{}{\coarseIdxtwo_p} \big),
    \end{equation}
    \vspace{-7mm}
...
The \textsc{Refine} step in the previous section produces the domain sequence $\domseq^{\mathrm{refine}}$. Crucially, $\domseq^{\mathrm{refine}}$ is not a admissible domain sequence because $\domseq^{\mathrm{refine}}$ might now contain inadmissible faces, pinch vertices, or boundary pinch vertices --- one for each clause of Definition~\ref{def:subdiv_partition}. The \textsc{Pad} step repairs these defects and turns $\domseq^{\mathrm{refine}}$ in the admissible domain sequence $\domseq^{\mathrm{pad}}$. The goal is to keep the padded faces added during this process to a minimum to avoid inflating the number of DoFs in non-marked regions. The only exception is some grading that we apply around vertices whose covering star covers too many levels. This grading does not change the restored admissibility properties but improves the conditioning of the resulting matrix-vector system. This section covers all three kinds of padding one by one.
...
\begin{equation}
    ...
    \guidance{\domseq^p = \quadriV{\l_p + 1} \big( \domseq^{p-1}, \, \vertex{\l_p}{\coarseIdxtwo_p} \big)} \quad \text{for} \quad \big(\l_p, \vertex{\l_p}{\coarseIdxtwo_p}\big) \in \mathrm{V}_{\domseq^{\mathrm{refine}}}^{\mathrm{pinch}} \quad \text{with} \quad \; p \in \{1, \, \dots, \, \vert \mathrm{V}_{\domseq^{\mathrm{refine}}}^{\mathrm{pinch}} \vert \},
\end{equation}
...
\begin{equation}
    ...
    \guidance{\domseq^p = \quadriV{\targetlevel_{\coarseIdxtwo_p} - 2} \big( \domseq^{p-1}, \, \vertex{}{\coarseIdxtwo_p} \big)} \quad \text{for} \quad \vertex{}{\coarseIdxtwo_p} \in \mathrm{V}_{\domseq^{\mathrm{admissible}}}^{\mathrm{grading}} \quad \text{with} \quad \; p \in \{1, \, \dots, \, \vert \mathrm{V}_{\domseq^{\mathrm{admissible}}}^{\mathrm{grading}} \vert \}
\end{equation}
and $\domseq^0 = \domseq^{\mathrm{admissible}}$. Note the additional $-2$ in the quadrisection operator $\quadriV{\targetlevel_{\coarseIdxtwo_p} - 2}$. It ensures that $\vertex{}{\coarseIdxtwo_p}$ has a level gap of $2$ or less in $\domseq^p$.

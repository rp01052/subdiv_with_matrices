...
\newcommand{\quadriV}{\mathcal{Q}^{\star}}                       % graded leaf-star quadrisection; args (domain sequence, vertex class, target level)
...
% Leaf star of a vertex READ AT LEVEL #1: the leaf faces covering the part of
% its level-#1 mesh star that is not refined beyond #1, hence carried at level
% #1 or coarser. Same slot as \FVl and \leafF, and \FVleaf{\l} \cap \F_\l is
...
\begin{definition}[Region]
    \label{def:region}
    A \emph{region} $\R_\l \subseteq \F_\l(\T_\l)$ is a collection of faces of $\T_\l$. We write $\R_{\l \to \lone} \coloneq \Aop{}{\l}{\lone} \R_\l$ for the region obtained by repeatedly splitting $\R_\l$ from level $\l$ to $\lone$. \robert{The set complement $\R^\complement_\l \coloneq \F_\l(\T_\l) \setminus \R_\l$ of $\R_\l$ is also a region.}
\end{definition}
...
Two operations on regions are used throughout and visualised in Figure~\ref{fig:region_anatomy}. \robert{We write $\FVl{\l}(\vertex{\l}{\coarseIdxone})$ for the faces of the entire mesh $\T_\l$ incident to a vertex $\vertex{\l}{\coarseIdxone}$, and $\V_\l(\R_\l)$ for the vertices of the faces ofthe region $\R_\l$.}

The first operation grows a region by one strip of faces. The \emph{extension}
...
\begin{definition}[Ancestor region]
    ...
    A region $\R_{\l+1} \subseteq \F_{\l+1}(\T_{\l+1})$ is \emph{quadrisection-closed} if $\R_{\l+1} = \R_{\l \to \l+1}$ for some region $\R_\l \subseteq \F_\l(\T_\l)$. On these regions, the \emph{ancestor} operator $\ancestorPatch{\cdot}$ is defined by
    \begin{equation}
...
\end{equation}
the \emph{leaf mesh} of $\domseq[\L]$. It is not a mesh in the strict sense because it can have hanging nodes and self-intersection for example. The leaf mesh is a useful tool to represent a domain sequence in plots, see for example Figure~\ref{fig:active_dofs}, with the understanding that the coarse parents of its faces are also in the domain sequence. 

The leaf mesh also tracks the refinement frontier in the sense that is a collection of the finest faces of a domain sequence that cover the faces of the domain $\T_\L$. Thus, the leaf mesh plays a large role in the definition of the refinement operators in Section~\ref{subsec:vertex_neighbourhoods}. 

\todo{\robert{The two halves of~\eqref{eq:leaf_ancestor_split} are easy to conflate, since a level carries both, and the distinction matters wherever a construction is restricted to a vertex or to a region. Intersecting with $\M_\l$ keeps the faces refined beyond $\l$, intersecting with $\leafF{\l}(\domseq[\L])$ discards them, and the two agree only where nothing is refined further.}}
...
\begin{figure}[tb]
    ...
    \cvsubfig
        ...
        {fig:adm_faces}
    \caption{The definition chain of this section read off a single domain sequence. Purple faces indicate the ancestor $\ancestorPatch{\M_{\l+1}}$ throughout, and thus marks the faces that are refined beyond the level we compute the predicates on. Panel~\subref{fig:adm_partition} shows the input the predicates use, namely the split~\eqref{eq:leaf_ancestor_split} of $\M_\l$ into the leaf faces $\leafF{\l}(\domseq)$ and the ancestor patch $\ancestorPatch{\M_{\l+1}}$. Panel~\subref{fig:adm_fan} uses that splitting to find the refinement fan $\refFan{\lzero}$ of a vertex, see Definition~\ref{def:refinement_fan}. Panel~\subref{fig:adm_exterior} marks every vertex of a different domain sequence with nested refinement by the level at which its refinement fan first becomes empty, i.e. from which it is refinement-exterior according to Definition~\ref{def:refinement_exterior_vertex}. \guidance{By~\eqref{eq:tau_is_first_full_level} that level is also the finest level $\targetlevel_\coarseIdxtwo$ of the vertex's leaf star, so the panel doubles as a plot of the level spans of Definition~\ref{def:level_span}.} Panel~\subref{fig:adm_faces} visualises the split of the leaf faces on level $0$ into admissible (green) and inadmissible (orange) by Definition~\ref{def:admissible_face}. The criterion is whether a leaf face contains at least one refinement-exterior vertex, drawn hollow.}
    \label{fig:admissibility_chain}
\end{figure}

\prose{Everything in this section is a condition on the domain sequence itself. The predicates below refer only to how its regions sit inside one another, never to how the sequence was produced, and they are exactly what the two theorems at the end of the section assume. A reader who wants to know which domain sequences carry well-behaved spaces needs this section and nothing after it.}

\prose{Section~\ref{subsec:selecting_active_dofs} selects the active DoFs of level $\l$ by the subtraction in~\eqref{eq:active_k_form_dofs}: the open DoFs of the extended region $\extendedPatch{\M}_\l$, less the open DoFs of the ancestor patch $\ancestorPatch{\M_{\l+1}}$, which the next level re-supplies at half the mesh size. Everything in this section follows from asking when that exchange is clean. It is clean as long as the part handed on, $\ancestorPatch{\M_{\l+1}}$, stays a minimum distance away from the collar that the extension adds around it. Where the two close in on one another, level $\l$ gives up DoFs that level $\l+1$ does not put back and the complex breaks; where coarse faces are enclosed by refinement instead, nothing coarse survives that the finer DoFs cannot already express and linear independence breaks.

The geometric predicates of this section are the local tests for the three ways that distance collapses: a leaf face can be surrounded by refinement at every one of its vertices, two refined parts of one vertex star can meet at that vertex alone, and at the domain boundary a refined part of a star can reach the boundary only through the vertex. All three are read off a single object, the \emph{refinement fan} of a vertex, which collects the faces of the mesh at that vertex that the sequence refines beyond the current level. We define it first, then take the three configurations in turn. That ruling them out suffices for both properties is proved in~\cite{companion_paper}; what matters here is that each test is local, on one vertex or one face.}
...
\prose{An empty fan is the local statement that the finer level does not reach a vertex at all, and it is what the first two configurations are read against.}
...
\prose{The fan can only empty out: once nothing incident to a vertex is refined beyond a level, nothing is on any finer level either, so a vertex that is refinement-exterior at one level stays refinement-exterior at all finer ones. Every vertex therefore has a first such level, which is what Figure~\ref{fig:adm_exterior} colours it by.}

\guidance{Note that the set of refinement-exterior vertices of $\domseq$ at a given level does not only contain the interior vertices of the patches of $\domseq$ but can also contain interface vertices and vertices on the boundary $\partial \domseq$. For example, Figure~\ref{fig:adm_exterior} displays a domain sequence with nested refinement up to level $3$ towards its centre, with every vertex coloured by the level from which it is refinement-exterior. The outermost layer is refinement-exterior already at level $0$, because none of its incident faces is refined beyond that level, so its fan (Definition~\ref{def:refinement_fan}) is empty there. The next layer becomes refinement-exterior only at level $1$, since at level $0$ it still has incident faces that are refined beyond that level. This logic applies to all layers in the figure until we reach the finest level $3$, at which the remaining vertices become refinement-exterior.}

\prose{The first configuration is a leaf face enclosed by refinement: if none of its vertices is refinement-exterior, the finer level reaches the face from every corner at once, and it is this case that costs linear independence. The predicate names the harmless complement.}

\begin{definition}[Admissible face]
    \label{def:admissible_face}
    A face $\face{\l}{\coarseIdxone} \in \F_\l(\domseq[\L])$ is \emph{admissible} if at least one of its vertices is refinement-exterior at level $\l$. Otherwise, it is \emph{inadmissible}.
\end{definition}

\prose{The second configuration is a vertex at which the refined part of the star falls into two or more pieces meeting only at the vertex itself. The coarse faces that separate them are squeezed to a point there, so the distance the subtraction relies on is lost, however wide the patches are elsewhere. Counting those pieces is what the next two definitions do.}
...
\prose{At the domain boundary the same collapse takes a second form that the number of pieces alone does not see: a single piece can be sealed off by coarse faces on both sides and touch the boundary only through the vertex.}
...
Condition (ii) is deliberately not restricted to interior vertices to cover cases like the one in Figure~\ref{fig:pinch_boundary_two_arcs}. \prose{The three conditions are the three collapses ruled out one by one, so together they are the minimum distance between the coarse region $\ancestorPatch{\M_{\l+1}}$ and its fine counterpart $\extendedPatch{\M}_{\l+1}$ that Section~\ref{subsec:selecting_active_dofs} needs, stated entirely in terms of one vertex or one face at a time.} The following two theorems are from ~\cite{companion_paper} and show that both linear independence and the correct cohomology of the adaptive subdivision $k$-form spaces follow from the admissibility of the domain sequence they are defined on.
...
\prose{The previous section constrained the domain sequence. This one and the next build the machinery that carries our adaptive loop from one admissible domain sequence to a finer one, so the definitions from here on are answerable to the algorithm rather than to the theorems: they are chosen so that the operators of Section~\ref{subsec:refinement_operators} can be stated and implemented, and a different refinement strategy would replace them while leaving Section~\ref{subsec:subdiv_partition} untouched. What the loop owes Section~\ref{subsec:subdiv_partition} is stated at the end of Section~\ref{subsec:refinement_operators}: refinement breaks admissibility, and the loop has to restore it.}

\guidance{The refinement and pinch resolution steps of the adaptive loop in Section~\ref{subsec:adaptive_loop} homogenise the domain sequence around a vertex, and have to reach it from whatever level the domain sequence currently covers it at. This subsection collects what that takes: a way to read one vertex on several levels, and a way to name the faces of the domain sequence that surround it. Because the meshes $\T_\l$ are nested, so are their vertex sets,}
\begin{equation}
...
\begin{definition}[Covering face]
    ...
    Let $\domseq[\L] \in \alldomseqs$ and let $\face{\lone}{\fineIdxone} \in \F_{\lone}(\T_\lone)$ be a face on level $\lone$. The \emph{covering face} of $\face{\l}{\fineIdxone}$ is exactly the leaf face $\face{\l}{\coarseIdxone} \in \leafMesh(\domseq[\L])$ iff $\face{\lone}{\fineIdxone} \in \Aop{}{\l}{\lone} \face{\l}{\coarseIdxone}$. We denote this realtionship by $\coveringFace{\face{\lone}{\fineIdxone}} = \face{\l}{\coarseIdxone}$. For $\l \leq \lone$, the covering face is unique because the leaf mesh $\leafMesh(\domseq[\L])$ is a tiling of the domain, and for $\l \!>\! \lone$, we define $\coveringFace{\face{\lone}{\fineIdxone}} = \varnothing$.
\end{definition}

\guidance{Such a step reads the domain sequence at a vertex through the leaf faces that touch it, at whatever levels they are carried.}

\begin{definition}[Leaf star]
    \label{def:leaf_star}
    Let $\domseq[\L] \in \alldomseqs$ and let $\vertex{\l}{\coarseIdxone} \in \V_\l(\T_\l)$ be a vertex on level $\l$. The \emph{leaf star} of $\vertex{\l}{\coarseIdxone}$ is the set of leaf faces of $\domseq[\L]$ covering the part of its mesh star $\FVl{\l}(\vertex{\l}{\coarseIdxone})$ that is not refined beyond level $\l$,
    \begin{equation}
        \label{eq:leaf_star}
        \FVleaf{} \big( \vertex{\l}{\coarseIdxone} \big) \coloneq \Big\{ \coveringFace \big( \face{\l}{\coarseIdxone} \big) \; \colon \;\; \face{\l}{\coarseIdxone} \in \FVl{\l} \big( \vertex{\l}{\coarseIdxone} \big) \setminus \refFan{\l} \big( \vertex{\l}{\coarseIdxone} \big) \Big\}.
    \end{equation}
    \todo{\robert{\robert{$\setminus \refFan{\l} \big( \vertex{\l}{\coarseIdxone} \big)$ not needed, right? covering faces of members of ref fan evaluate to $\varnothing$ because leaf faces are finer than $\l$?}}}
    \guidance{Removing the refinement fan is exactly what makes $\coveringFace$ defined on the remaining faces. On level $\L$ no face is refined beyond the level, so $\refFan{\L} = \varnothing$ and the restriction is vacuous.}
\end{definition}

The members of $\FVleaf{}(\vertex{\l}{\coarseIdxone})$ are the leaf faces of $\domseq[\L]$ carried at level $\l$ or coarser that touch $\vertex{\l}{\coarseIdxone}$, so they are pairwise disjoint, and those of them carried at level $\l$ itself are exactly
\begin{equation}
    \label{eq:leaf_star_slice}
    \robert{\FVleaf{\l} \big( \vertex{\l}{\coarseIdxone} \big) \coloneq \FVleaf{} \big( \vertex{\l}{\coarseIdxone} \big)\cap \F_\l(\T_\l) \; = \; \FVl{\l} \big( \vertex{\l}{\coarseIdxone} \big) \cap \leafF{\l}(\domseq[\L]).}
\end{equation}
Equivalently, $\FVleaf{}(\vertex{\l}{\coarseIdxone})$ collects the faces of the leaf mesh at level $\l$ or coarser that touch the vertex. It grows with $\l$ and stops growing once $\l$ reaches the finest level at which a leaf face touches the vertex. From there on, it tiles a whole neighbourhood of it in the leaf mesh. Note that the covering face operator from Definition~\ref{def:covering_face} tests all levels from $\lzero$ to $\l$, so a coarse leaf face carrying the vertex $\vertex{\l}{\coarseIdxone}$ in the interior of one of its edges is a member of the leaf star like any other, and is refined like any other. Unlike the refinement fan of Definition~\ref{def:refinement_fan}, which is read at a single level, the leaf star reaches down through every coarser level, which is what the refinement and pinch resolution steps have to act on. \robert{(point towards the right figure, we have that already.)}

The refinement fan and the leaf star are the two halves of one decomposition. At every level the mesh star $\FVl{\l}(\vertex{\l}{\coarseIdxone})$ of $\vertex{\l}{\coarseIdxone} \in \robert{\V_\l(\T_\l)}$ divides into the faces the sequence refines beyond $\l$ and the faces it does not, and~\eqref{eq:leaf_star} sends the second half through $\coveringFace$ to the leaf star:
\begin{equation}
    \label{eq:star_splits}
    \FVl{\l} \big( \vertex{\l}{\coarseIdxone} \big) \; = \; \refFan{\l} \big( \vertex{\l}{\coarseIdxone} \big) \; \sqcup \; \robert{\coveringFace^{-1}} \Big( \FVleaf{} \big( \vertex{\l}{\coarseIdxone} \big) \Big),
\end{equation}
which is the split~\eqref{eq:leaf_ancestor_split} of a level read at one vertex. The two halves are not symmetric: the fan is a set of faces on level $\l$, while the leaf star names the covering faces of the other half and therefore lives at levels $\l$ and coarser, so its members can extend well beyond the mesh star on level $\l$.

\prose{Figure~\ref{fig:leaf_star_levels} follows a single vertex through~\eqref{eq:star_splits}. Read across the row the fan gives way to the leaf star: what is refined beyond the level being read shrinks, what the leaf star covers grows, and the two together account for the neighbourhood at every level.}
...
\begin{figure}[tb]
    ...
    \cvsubfig[0.2317]
        ...
        {The leaf mesh of $\domseq$ and the vertex the other panels read}
        {fig:leafstar_overview}
    \hfill
    \cvsubfig[0.235]
        {images/visualisations/v8_graded_top_bottom_sides_leafstar_lvl0.pdf}
        {$\l = \lzero$: six of the eight star faces are refined beyond the level, two are leaf faces}
        {fig:leafstar_l0}
    \hfill
    \cvsubfig[0.235]
        {images/visualisations/v8_graded_top_bottom_sides_leafstar_lvl1.pdf}
        {$\l = 1$: two are refined beyond the level, and two coarse leaf faces reach outside the star}
        {fig:leafstar_l1}
    \hfill
    \cvsubfig[0.235]
        {images/visualisations/v8_graded_top_bottom_sides_leafstar_lvl2.pdf}
        {$\l = 2$: none is, so the leaf star is full and covers the star from three levels at once}
        {fig:leafstar_l2}
    \caption{\guidance{One vertex read at every level, with its mesh star split by~\eqref{eq:star_splits}. Panel~\subref{fig:leafstar_overview} is the leaf mesh of $\domseq$, what the domain sequence actually carries, with the vertex marked. Each of the other panels shows the whole uniform mesh $\T_\l$ of its level, drawn in the same level shading as the leaf mesh so that the background alone says which level is being read, with the eight faces of the mesh star $\FVl{\l}$ of that vertex outlined. Inside the outline the split is coloured: the refinement fan $\refFan{\l}$ in purple, keeping the meaning purple has in Figures~\ref{fig:admissibility_chain} and~\ref{fig:pinches}, and the leaf star $\FVleaf{\l}$ in blue, each of its members outlined so that they can be told apart where several meet. No panel is zoomed, so the star is seen to shrink by one level from panel to panel against a fixed frame. The two colours sit differently on the mesh, and that is the asymmetry of~\eqref{eq:star_splits}: the fan is drawn on level-$\l$ faces and stays inside the outline, while the leaf star is drawn on covering faces, at whatever level they are carried, so a single coarse member covers several star faces at once and extends beyond the star. Reading across, the fan empties while the leaf star fills, and the first level at which nothing is refined beyond it is $\targetlevel_\coarseIdxtwo = 2$, where the leaf star covers the star with faces from levels $\lzero$, $1$ and $2$ together.}}
    \label{fig:leaf_star_levels}
\end{figure}

\begin{definition}[Full leaf star]
    \label{def:full_leaf_star}
    The leaf star $\FVleaf{}(\vertex{\l}{\coarseIdxone})$ is \emph{full at level $\l$} if it covers the whole mesh star on level $\l$, that is, if $\coveringFace(\face{\l}{\fineIdxone}) \neq \varnothing$ for every $\face{\l}{\fineIdxone} \in \FVl{\l}(\vertex{\l}{\coarseIdxtwo})$.
\end{definition}

A leaf star is full exactly when its vertex is refinement-exterior at that level in the sense of Definition~\ref{def:refinement_exterior_vertex} because the faces the leaf star misses are precisely the ones the refinement fan holds. Thus, we can use fullness of a leaf star to read off whether a vertex is refinement-exterior in a leaf mesh, cf. Figure~\ref{fig:adm_exterior}.

\begin{definition}[Level span of a vertex]
    \label{def:level_span}
    \guidance{Let $\domseq[\L] \in \alldomseqs$ and let $\vertex{}{\coarseIdxtwo} \in \mathrm{V}_{\domseq}$, by which we mean the vertices that are a vertex of at least one leaf face of $\domseq[\L]$. We write
    \begin{equation}
        \label{eq:level_span}
        \targetlevel_\coarseIdxtwo \coloneq \min \big\{ \l \; \colon \; \refFan{\l}(\vertex{\l}{\coarseIdxtwo}) = \varnothing \big\},
        \qquad
        \sourcelevel_\coarseIdxtwo \coloneq \min \big\{ \l \; \colon \; \FVleaf{\L}(\vertex{\L}{\coarseIdxtwo}) \cap \F_\l(\T_\l) \neq \varnothing \big\}
    \end{equation}
    for the level from which $\vertex{}{\coarseIdxtwo}$ is refinement-exterior and for the coarsest level occurring in its leaf star, and call $\mu_\coarseIdxtwo \coloneq \targetlevel_\coarseIdxtwo - \sourcelevel_\coarseIdxtwo$ the \emph{level gap} of $\vertex{}{\coarseIdxtwo}$.}
\end{definition}

\guidance{The two ends of the span are read off different objects, and only the coarse one needs the leaf star. Because the fan can only empty out, $\targetlevel_\coarseIdxtwo$ is equivalently the first level at which the leaf star is full, and also the finest level the leaf star occupies,
\begin{equation}
    \label{eq:tau_is_first_full_level}
    \targetlevel_\coarseIdxtwo \; = \; \min \big\{ \l \; \colon \; \FVleaf{\l}(\vertex{\l}{\coarseIdxtwo}) \text{ is full} \big\} \; = \; \max \big\{ \l \; \colon \; \FVleaf{\L}(\vertex{\L}{\coarseIdxtwo}) \cap \F_\l(\T_\l) \neq \varnothing \big\},
\end{equation}
so it can be taken off Figure~\ref{fig:adm_exterior} directly. The restriction to $\mathrm{V}_{\domseq}$ is what keeps the three readings together: a vertex created inside a coarse leaf face is refinement-exterior as soon as it exists, while its leaf star never reaches that level, and the level gap would report a spread that no refinement put there. The coarse end $\sourcelevel_\coarseIdxtwo$ admits no fan reading at all, since it may lie below the level at which the vertex is created, where its mesh star, and hence its fan, is not defined.}

\subsection{Refinement operators}
\label{subsec:refinement_operators}
\label{subsec:operations_on_coverings}

\guidance{The remainder of this section introduces operators that enable the kinds of refinement that we will later need for the adaptive loop in Section~\ref{subsec:adaptive_loop}. The \emph{face quadrisection operator} $\quadriF(\domseq, \simplexsubset{2}{\l})$ refines a \emph{single-level} set of faces $\simplexsubset{2}{\l} \subseteq \F(\domseq_\l)$ to level $\l\!+\!1$ by replacing each face with its four children, that is, it removes the faces $\simplexsubset{2}{\l}$ from level $\l$ and adds their children on level $\l+1$, leaving all other levels untouched.
...
\guidance{The face quadrisection operator refines a flat set of faces on a single level. The graded operator below instead homogenises a whole leaf star, and it does so by applying $\quadriF$ repeatedly, one level at a time.}

\begin{definition}[Graded vertex quadrisection operator]
    \label{def:graded_vertex_quadrisection}
    \guidance{Let $\domseq[\L] \in \alldomseqs$, let $\vertex{\l}{\coarseIdxtwo} \in \V_\l(\T_\l)$ and let $\targetlevel \leq \l + 1$ be a target level. The \emph{graded vertex quadrisection operator}
    \begin{equation}
        \quadriV \; \colon \; \alldomseqs \times \V \times \{ \lzero, \hdots, \L \} \; \longrightarrow \; \alldomseqs
    \end{equation}
    homogenises the leaf star of $\vertex{\l}{\coarseIdxtwo}$ at level $\targetlevel$: for $\l' = \sourcelevel_\coarseIdxtwo, \hdots, \targetlevel - 1$ in ascending order, apply
    \begin{equation}
        \label{eq:vertex_quadrisection}
        \domseq[\L] \; \longmapsto \; \quadriF \Big( \domseq[\L], \, \FVleaf{\l} \big( \vertex{\l}{\coarseIdxtwo} \big) \cap \F_{\l'}(\T_{\l'}) \Big),
    \end{equation}
    the leaf star being re-read after each step.}
\end{definition}

\guidance{Each step of~\eqref{eq:vertex_quadrisection} raises to $\l'+1$ the level of every leaf-star face carried at $\l'$, and by~\eqref{eq:leaf_star_slice} the children it creates are the leaf-star faces the next step reads at $\l'+1$. The sweep therefore terminates after $\targetlevel - \sourcelevel_\coarseIdxtwo$ applications of the face quadrisection operator and drags a coarse leaf up through \emph{every} intermediate level: a level-$\lzero$ face beside a level-$2$ arc is lifted twice. Reading the star at level $\l$ is what keeps the operator local in level: everything the domain sequence already carries finer than $\l$ sits in the refinement fan and is never a member, so a second application finds an empty star and the operator is idempotent. The target level sits in the argument slot rather than on the operator, since $\quadriV$ is one map that the two steps using it, \textsc{Refine} and \textsc{Pad}, call with different targets; they are described in Sections~\ref{subsec:refine} and~\ref{subsec:padding}.}

The quadrisection operators defined in this section are the basis upon which we will later built the algorithms in the adaptive loop in Section~\ref{subsec:adaptive_loop}. They enable consistent refinement of the adaptive subdivision $k$-form spaces $\hierSDFspace{k}{\domseq}{\L}$ in the sense that
\begin{equation}
    \label{eq:quadrisection_monotone}
    \hierSDFspace{k}{\domseq}{\L} \subseteq \hierSDFspace{k}{\quadriF(\domseq[\L], \simplexsubset{2}{\l})}{} \qquad \text{and} \qquad \hierSDFspace{k}{\domseq}{\L} \subseteq \hierSDFspace{k}{\quadriV(\domseq[\L], \vertex{\l}{\coarseIdxtwo}, \targetlevel)}{}.
\end{equation}
The reason the refinement produces proper superspaces is that the domain sequence are nested in the sense that $\M_\l \subseteq \tilde{\M}_\l$, where $\M_\l$ and $\tilde{\M}_\l$ are the regions of $\domseq[\L]$ and $\quadriF(\domseq[\L], \simplexsubset{2}{\l})$ or $\quadriV(\domseq[\L], \vertex{\l}{\coarseIdxtwo}, \targetlevel)$, respectively. \cite{companion_paper} prove the nestedness of the adaptive $k$-form spaces given that nestedness of regions.

% --- Figure knobs: leaf star and its quadrisection. Moved here from
% Section~\ref{subsec:subdiv_partition}: this is the only place these panels are
% cited, and unlike there, the leaf star has by now been defined, by
% Equation~\eqref{eq:leaf_star} above. Reuses \cvsubfig, still defined in
% the earlier section. The labels and the image filenames still say
% "covering_star" -- the figure is generated under that name, so they track the
% files rather than the prose. -----------------------------------------------
\begin{figure}[tb]
    \centering
    \cvsubfig[0.32]
        {images/visualisations/neighbour-neighbour_input_with_covering_star.pdf}
        {The leaf faces of $\domseq$ incident to a vertex $\vertex{}{\coarseIdxtwo}$, carried at mixed levels}
        {fig:covering_star}
    \hfill
    \cvsubfig[0.32]
        {images/visualisations/neighbour-neighbour_refined_with_covering_star.pdf}
        {The star from~\subref{fig:covering_star} after applying $\quadriV(\domseq, \vertex{\L}{\coarseIdxtwo}, \targetlevel_\coarseIdxtwo \! + \! 1)$}
        {fig:covering_star_refined}
    \caption{\guidance{The leaf star~\eqref{eq:leaf_star} of a vertex $\vertex{}{\coarseIdxtwo}$ sitting on a level interface, and the effect of the graded vertex quadrisection operator on it. Unlike the refinement fan of Definition~\ref{def:refinement_fan}, which is read at a single level, the leaf star collects the incident leaf faces at whatever levels they are carried, which is why it spans the interface in~\subref{fig:covering_star}.}}
    \label{fig:covering_star_fig}
\end{figure}

\guidance{Re-reading the leaf star after every step is what drags the coarse faces up through \emph{every} intermediate level. The two operators are distinguished by their superscript: $\quadriF$ refines a set of faces flatly from level $\l$ to $\l\!+\!1$, while $\quadriV$ homogenises a leaf star towards the target level it is handed. Figure~\ref{fig:covering_star_refined} displays the result of applying $\quadriV(\domseq, \vertex{\L}{\coarseIdxtwo}, \targetlevel_\coarseIdxtwo \! + \! 1)$ to the star of Figure~\ref{fig:covering_star}: the neighbourhood is first brought up to the finest level $\targetlevel_\coarseIdxtwo$ that $\vertex{}{\coarseIdxtwo}$ already carries, and then one level further.}

\guidance{Admissible domain sequences respect the geometric constraints necessary for linear independence and reproducing the correct cohomology. Unfortunately, in contrast to general domain sequences $\allcoverings$, admissible domain sequences are not closed under the quadrisection operator $\quadriF(\domseq, \cdot)$ and $\quadriV(\domseq, \cdot, \targetlevel)$, meaning that they are maps $\allpartitions \to \allcoverings$. Thus, naive refinement of admissible domain sequences can reintroduce inadmissible geometry, for example a pinch vertex. This is exactly why Section~\ref{subsec:adaptive_loop} sets up a padding step that repairs these geometric defects and turns the domain sequence into a admissible domain sequence again.}
...
Our code marks vertices rather than faces. This enables us to use the quadrisection operator $\quadriV$ to refine single vertex stars, the central combinatorial object of the admissibility arguments in~\cite{companion_paper}. We rank the vertices by the vertex estimator $\nu_\coarseIdxtwo$, see Equation~\eqref{eq:vertex_residual} from Section~\ref{subsec:estimate}. For this purpose, we enumerate the $\vert \mathrm{V}_{\domseq} \vert$ vertices $\vertex{}{\coarseIdxtwo} \in \mathrm{V}_{\domseq}$ in order of decreasing residual, so that $\nu_{\coarseIdxtwo_1} \geq \nu_{\coarseIdxtwo_2} \geq \ldots \geq \nu_{\coarseIdxtwo_{\mathrm{V}-1}} \geq \nu_{\coarseIdxtwo_{\vert \mathrm{V}_{\domseq} \vert}}$.

The Dörfler marking algorithm\needcite{} builds the set $\mathrm{V}_{\domseq}^\mathrm{mark} \coloneq \{ \vertex{}{\coarseIdxtwo_1}, \, \dots, \, \vertex{}{\coarseIdxtwo_q} \}$ by greedily selecting the first $q$ vertices with the largest estimators such that the \emph{marked refinable residual}
\begin{equation}
...
The \textsc{Refine} step refines the marked faces $\face{\l}{\coarseIdxone} \in \R_\theta$ around the vertices $\vertex{}{\coarseIdxtwo} \in \mathrm{V}_{\domseq}^{\mathrm{mark}}$ from Equation~\eqref{eq:marked_refinable_residual} as shown in Figure~\ref{fig:pipeline_marking_c}. Recall from Definition~\ref{def:level_span} that $\sourcelevel_\coarseIdxtwo$ and $\targetlevel_\coarseIdxtwo$ are the coarsest and the finest level occurring in the leaf star of $\vertex{}{\coarseIdxtwo}$ in $\domseq$. Since we want to refine the faces around the vertex by one level (unless $\targetlevel_\coarseIdxtwo = \L$), we compute the target level $\tilde{\targetlevel}_\coarseIdxtwo = \min(\targetlevel_{\coarseIdxtwo_p} \! + \! 1, \L)$ that is expected after refining around $\vertex{}{\coarseIdxtwo}$. The expected target levels $\tilde{\targetlevel}_\coarseIdxtwo$ must be computed on the fixed input admissible domain sequence $\domseq$ before any refinement takes place to avoid a cascade of ever increasing target levels. 

The \textsc{Refine} step homogenises the star of each marked vertex $\vertex{}{\coarseIdxtwo} \in \mathrm{V}_{\domseq}^{\mathrm{mark}}$ at one level beyond the finest level it currently carries. Figure~\ref{fig:refine_interface} displays two examples of this refinement for vertices at an interface. Formally, we express the refinement through the graded vertex quadrisection $\quadriV(\domseq, \vertex{\L}{\coarseIdxtwo}, \tilde{\targetlevel}_\coarseIdxtwo)$ of Definition~\ref{def:graded_vertex_quadrisection}.
...
\begin{enumerate}[topsep=3pt, itemsep=-2pt]
    ...
    \item refine vertex stars iteratively for all $\vertex{}{\coarseIdxtwo_p} \in \mathrm{V}_{\domseq}^{\mathrm{mark}}$ with $p = 1, \, \dots, \, \vert \mathrm{V}_{\domseq}^{\textrm{mark}} \vert$ to obtain
    \begin{equation}
        \domseq^{p} = \quadriV \big( \domseq^{p-1}, \, \vertex{\L}{\coarseIdxtwo_p}, \, \tilde{\targetlevel}_{\coarseIdxtwo_p} \big),
    \end{equation}
    \vspace{-7mm}
    \item set $\domseq^{\mathrm{refine}} \coloneq \domseq^{\vert \mathrm{V}_{\domseq}^{\textrm{mark}} \vert}$.
\end{enumerate}
Because the expected target levels $\tilde{\targetlevel}_{\coarseIdxtwo}$ are computed on the fixed input domain sequence $\domseq$ and each quadrisection only ever homogenises a star upward to $\tilde{\targetlevel}_{\coarseIdxtwo}$, the order in which the $\vertex{}{\coarseIdxtwo} \in \mathrm{V}_{\domseq}^{\mathrm{mark}}$ are processed does not affect the resulting partition $\domseq^p$. We may therefore apply them in any order; our code processes the coarsest vertices first.
...
The \textsc{Pad} step needs no machinery beyond the two quadrisection operators $\quadriF$ and $\quadriV$ of Section~\ref{subsec:refinement_operators}, since every padding substep is a quadrisection of either a vertex star or a set of inadmissible faces.
...
\begin{equation}
    ...
    \guidance{\domseq^p = \quadriV \big( \domseq^{p-1}, \, \vertex{\l_p}{\coarseIdxtwo_p}, \, \l_p \! + \! 1 \big)} \quad \text{for} \quad \big(\l_p, \vertex{\l_p}{\coarseIdxtwo_p}\big) \in \mathrm{V}_{\domseq^{\mathrm{refine}}}^{\mathrm{pinch}} \quad \text{with} \quad \; p \in \{1, \, \dots, \, \vert \mathrm{V}_{\domseq^{\mathrm{refine}}}^{\mathrm{pinch}} \vert \},
\end{equation}
...
\begin{equation}
    ...
    \guidance{\domseq^p = \quadriV \big( \domseq^{p-1}, \, \vertex{\L}{\coarseIdxtwo_p}, \, \targetlevel_{\coarseIdxtwo_p} \! - \! 2 \big)} \quad \text{for} \quad \vertex{}{\coarseIdxtwo_p} \in \mathrm{V}_{\domseq^{\mathrm{admissible}}}^{\mathrm{grading}} \quad \text{with} \quad \; p \in \{1, \, \dots, \, \vert \mathrm{V}_{\domseq^{\mathrm{admissible}}}^{\mathrm{grading}} \vert \}
\end{equation}
and $\domseq^0 = \domseq^{\mathrm{admissible}}$. Note the additional $-2$ in the target level $\targetlevel_{\coarseIdxtwo_p} \! - \! 2$ handed to the quadrisection operator. It ensures that $\vertex{}{\coarseIdxtwo_p}$ has a level gap of $2$ or less in $\domseq^p$.

\todo{\guidance{The grading is stated here as a single application of $\quadriV$ with target level $\targetlevel_\coarseIdxtwo \! - \! 2$, i.e.\ by its outcome. The implementation instead lifts a vertex's \emph{coarsest} active faces by one level and re-tests, iterating until the gap is at most $2$, and re-runs the admissibility pad after each round. The two agree at the fixed point --- the iteration only ever raises $\sourcelevel_\coarseIdxtwo$ towards $\targetlevel_\coarseIdxtwo - 2$ --- so this is a question of whether to describe the loop or its result, not a discrepancy. Decide before submission.}}
...
\begin{problem}[Uniformly bounded restoration of admissibility]
    ...
    operators $\quadriF$ and $\quadriV$ whose marked sets are determined by the
...
\begin{proof}
    ...
    and are not leaf faces, so $\quadriV$ with target level $\l\!+\!1$ performs at least one face
    quadrisection. The same
...
partitions. That means that the defects caused by the spurious $1$-form harmonics of $\hierSDFspaceZero{1}{\domseq^{\mathrm{pinch}}}{\L}$ are confined to the eigenspace of $\lambda_{h,\coarseIdxone} = 0$. The reason for that behaviour is Theorem~\ref{theorem:differential_complex}, stating that $\extd \circ \extd \equiv 0$ holds pointwise on $\hierSDFspaceZero{0}{\domseq}{\L}$ for all domain sequences $\domseq \in \allcoverings$, regardless of whether they satisfy the admissible domain sequence conditions. This implies that $\extd u$ does not carry a spurious curl of any gradient, and thus the non-zero spectrum does not get polluted, cf.~\cite{Boffi.2010}.

...
The vertices $\V(\domseq[\L])$ of the domain sequence $\domseq[\L] = \bigsqcup_{\l = \lzero}^{\L} \M_\l$ given by
\begin{equation}
...
Condition (ii) is deliberately not restricted to interior vertices to cover cases like the one in Figure~\ref{fig:pinch_boundary_two_arcs}. The three conditions are the three collapses ruled out one by one, so together they are the minimum distance between the coarse region $\ancestorPatch{\M_{\l+1}}$ and its fine counterpart $\extendedPatch{\M}_{\l+1}$ that Section~\ref{subsec:selecting_active_dofs} needs, stated entirely in terms of one vertex or one face at a time. 

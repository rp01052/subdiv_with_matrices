...
% \quadriV asks for; \detectF eats a candidate FACE set and returns the family
% \quadriF asks for. Pi is free (no \prod in the paper) and reads as pinch.
\newcommand{\detectV}{\Pi^{\star}}                               % pinch detection on vertices
\newcommand{\detectF}{\Pi^{\triangle}}                           % confined-face detection
...
\begin{definition}[\robert{Candidate collection}]
    ...
    \end{equation}
    onto the vertices of the faces that $\quadriF(\domseq[\L], \simplexsubset{2}{})$ refines for the first time, each read at the level of its face.}
\end{definition}

\robert{By Lemma~\ref{lemma:pad_locality} no vertex outside~\eqref{eq:candidate_collection} carries a pinch or a boundary pinch of $\quadriF(\domseq[\L], \simplexsubset{2}{})$ that it did not already carry in $\domseq[\L]$. By Lemma~\ref{lemma:new_vertices} the vertices the quadrisection adds need no separate test, since case $(i)$ puts such a vertex into~\eqref{eq:candidate_collection} on its own and case $(ii)$ leaves nothing to test. The superscript is the one $\quadriV$ carries, and the face counterpart $\collectF$ belongs to the confinement substep, where Lemma~\ref{lemma:confined_children} narrows the same collection to the children of the refined faces.}

\guidance{Two things to check before this is final. The subtraction of $\ancestorPatch{\M_{\l+1}}$ in~\eqref{eq:candidate_collection} is new. The old candidate set was $\V_\l(\simplexsubset{2}{\l})$ over the whole slice, which is strictly larger whenever a marked class star reaches into a region that is already refined, and that happens in every cycle after the first. Confirm that the implementation subtracts as well before the counts of Section~\ref{sec:numerical_experiments} are read against this. The other point is $\collectF$. It is defined as a macro but not used, because the confinement candidates of~\eqref{eq:confined_candidates} are the children of the refined faces and not the vertices of them, so the two members of the family do not share a formula, only a role.}
...
\begin{definition}[\robert{Pinch detection}]
    ...
    \begin{equation}
        ...
        \pinchClasses \; \coloneq \; \big\{ \vertexClass{\coarseIdxone} \in \vertexClasses(\domseq[\L]) \; \colon \;\; \vertex{\l}{\coarseIdxone} \in \V^{\mathrm{pinch}}(\domseq[\L]) \, \cap \, \V^{\mathrm{cand}} \;\text{ for at least one level }\; \l \big\},
    \end{equation}
    and the target level map on them,
    \begin{equation}
        \label{eq:pinch_detection_target}
        \targetlevel_\coarseIdxone \; \coloneq \; \max \big\{ \, \l \! + \! 1 \; \colon \;\; \vertex{\l}{\coarseIdxone} \in \V^{\mathrm{pinch}}(\domseq[\L]) \, \cap \, \V^{\mathrm{cand}} \big\} .
    \end{equation}
    The set in~\eqref{eq:pinch_detection_target} is non-empty for every $\vertexClass{\coarseIdxone} \in \pinchClasses$ by~\eqref{eq:pinch_detection_classes}, so the maximum is always taken over at least one level.}
\end{definition}
...
Similarly to resolving pinches, we iterate over all leaf faces $\face{\l}{\coarseIdxone}$ of a domain sequence $\domseq[\L]$ to build the set of confined faces
\begin{equation}
    \label{eq:confined_set}
    \F^{\mathrm{confined}}(\domseq[\L]) = \big\{ \face{\l}{\coarseIdxone} \in \leafF{\l}(\domseq[\L]) \; \colon \; \face{\l}{\coarseIdxone} \text{\; is confined} \big\}
\end{equation}
according to Definition~\ref{def:confined_face}. \robert{A set $\F^\star \subseteq \F^{\mathrm{confined}}(\domseq[\L])$ of detected confined faces is repaired in one application of the face quadrisection,}
\begin{equation}
    \label{eq:confined_faces_resolved_subdiv_partition}
    \quadriF \big( \domseq[\L], \, \F^\star \big).
\end{equation}
...
\robert{In words, we refine every confined face by one level. That does not in general leave a domain sequence without confined faces, because the children of a confined face on level $\l$ inherit its neighbourhood. \robert{Suppose} that neighbourhood is refined two or more levels beyond $\l$. \robert{Then} the child at the corner shared by the finer faces has no refinement-exterior vertex on level $\l\!+\!1$ either and is confined in turn. The substep is therefore iterated in the same shape as the pinch resolution\robert{.}
\begin{enumerate}[topsep=3pt, itemsep=-2pt]
    \item initialise $\domseq[\L]^{\mathrm{confined},0} \coloneq \domseq[\L]^{\mathrm{pinch}}$ and the candidate set $\F^{\mathrm{cand},0} \coloneq \leafMesh(\domseq[\L]^{\mathrm{pinch}})$ to all leaf faces on all levels,
    \item in round $k \geq 1$, detect the confined faces among the candidates on the unmutated $\domseq[\L]^{\mathrm{confined},k-1}$,
    \begin{equation}
        \label{eq:confined_round_detection}
        \F^{\mathrm{confined},k} \coloneq \F^{\mathrm{confined}} \big( \domseq[\L]^{\mathrm{confined},k-1} \big) \, \cap \, \F^{\mathrm{cand},k-1} ,
    \end{equation}
    \vspace{-7mm}
    \item refine them \tobecut{level by level }as in~\eqref{eq:confined_faces_resolved_subdiv_partition} with $\F^\star = \F^{\mathrm{confined},k}$ started from $\domseq[\L]^{\mathrm{confined},k-1}$, to obtain $\domseq[\L]^{\mathrm{confined},k}$, and take the children of the faces just refined as the candidates of the next round\robert{, writing $\F^{\mathrm{confined},k}_\l$ for the level-$\l$ slice},
    \begin{equation}
        \label{eq:confined_candidates}
        \F^{\mathrm{cand},k} \coloneq \bigsqcup\limits_{\l = \lzero}^{\L-1} \Aop{}{\l}{\l+1} \big( \F^{\mathrm{confined},k}_\l \big),
    \end{equation}
    \vspace{-7mm}
    \item stop at the first round with $\F^{\mathrm{confined},k} = \varnothing$ and set $\domseq[\L]^{\mathrm{confined}} \coloneq \domseq[\L]^{\mathrm{confined},k}$.
\end{enumerate}}
...
\begin{lemma}[\robert{Confinement padding confines only its own children}]
    ...
    \robert{Let $\domseq[\L] \in \alldomseqs$, let $\F^\star \subseteq \F^{\mathrm{confined}}(\domseq[\L])$ be a set of confined leaf faces, and let $\domseq[\L]' \coloneq \quadriF(\domseq[\L], \F^\star)$ be the result of refining them as in~\eqref{eq:confined_faces_resolved_subdiv_partition}. Then every leaf face of $\domseq[\L]'$ that is confined in $\domseq[\L]'$ and was not confined in $\domseq[\L]$ is a child of a face of $\F^\star$.}
\end{lemma}
\begin{proof}
    % SUPERSEDED (2026-08-25): the sweep over levels, and with it the split into
    % the two cases l' = l and l' != l, was an artefact of reading a single
    % multi-level application one level at a time. Lemma~\ref{lemma:pad_locality}
    % is stated for a whole marked family, so the argument runs once, on the level
    % of the face. Kept for cherry-picking.
    \tobecut{The regions of a domain sequence only grow under quadrisection, so a refinement-exterior vertex can only lose that property and a confined face stays confined. The hypothesis therefore still holds at every level of the sweep, and it suffices to consider the refinement of the faces of $\F^{\mathrm{confined}}$ on one level $\l$.
    ...
    \robert{Let $\face{\lone}{\fineIdxone}$ be a leaf face of $\domseq[\L]$ that is unconfined in $\domseq[\L]$ and is still a leaf face of $\domseq[\L]'$, and let $\vertex{\lone}{\coarseIdxtwo}$ be one of its vertices that is refinement-exterior on level $\lone$. Then $\vertex{\lone}{\coarseIdxtwo}$ is a vertex of no face of the slice $\F^\star_\lone$. Such a face is confined and therefore has no refinement-exterior vertex on level $\lone$ by Definition~\ref{def:confined_face}, whereas the fan of $\vertex{\lone}{\coarseIdxtwo}$ is empty there. Lemma~\ref{lemma:pad_locality} therefore leaves $\refFan{\lone}(\vertex{\lone}{\coarseIdxtwo})$ unchanged, the vertex stays refinement-exterior and $\face{\lone}{\fineIdxone}$ stays unconfined. The only faces of $\domseq[\L]'$ that are not faces of $\domseq[\L]$ are the children of $\F^\star$.}
\end{proof}
...
\begin{corollary}[\robert{The confinement iteration advances one level per round}]
    ...
    \end{equation}
    The iteration performs $q^{\mathrm{confined}} \! + \! 1$ rounds and returns a domain sequence $\domseq[\L]^{\mathrm{confined}}$ without confined leaf faces, that is, one satisfying clause~(i) of Definition~\ref{def:subdiv_partition}.}
\end{corollary}
\begin{proof}
    \robert{By Lemma~\ref{lemma:confined_children} the set $\F^{\mathrm{confined},k+1}$ is contained in the candidate set $\F^{\mathrm{cand},k}$ of~\eqref{eq:confined_candidates}, whose faces are children of faces of $\F^{\mathrm{confined},k}$ and hence lie on levels $\geq \l_k \! + \! 1$. No leaf face on level $\L$ is ever confined, since every vertex is refinement-exterior on level $\L$ by Definition~\ref{def:refinement_fan}. Hence $\l_k \leq \L \! - \! 1$ for every productive round, which gives~\eqref{eq:confinement_rounds}. An empty round certifies that no leaf face is confined, since by Lemma~\ref{lemma:confined_children} every face whose confinement has changed since the previous detection is a candidate.}
\end{proof}

\robert{The cost of the iteration is therefore bounded by the number of levels rather than by the size of the mesh, and after the first round each round only tests the children of the faces the previous one refined. \robert{Call $d$ the burial depth of a confined face on level $\l$, the number of levels by which its neighbourhood is refined beyond $\l$. Then $q^{\mathrm{confined}}$ is the largest burial depth in $\F^{\mathrm{confined},1}$. In particular $q^{\mathrm{confined}} = 1$ exactly when no confined face has $d > 1$, which is the case at all but one step of the adaptive runs of Section~\ref{subsec:vector_laplace}.}}

\guidance{Measured, for the discussion of the bound. A confined face whose neighbourhood is refined $d$ levels beyond it costs exactly $d$ rounds, verified for $d = 1, \dots, 4$, and the output is a staircase in which each round refines only the corner child at the buried vertex. In the adaptive runs the deepest occurrence is $d = 2$, at one step of the $\L = 3$ run, and $d = 1$ everywhere else. Worth settling before it is claimed: with the grading moved to the front, the input to the first cycle has span at most $g\!+\!1$ by the paragraph after~\eqref{eq:grading_subdiv_partition}, which would give $d \leq g\!+\!2$ and hence a constant bound $q^{\mathrm{confined}} \leq g\!+\!2 = 4$ rather than~\eqref{eq:confinement_rounds}. Two things are still missing. The first is that the bound has to survive the pinch resolution that runs between the grading and the confinement padding, which is the same accumulation question as the one left open after~\eqref{eq:grading_subdiv_partition}. The second is that later cycles receive the output of the previous one and not the graded sequence, so the span they see is only bounded if that question is settled too. On a hand-built input of depth $4$ with $g = 2$ the round count went up rather than down, which is what the reordering has to be checked against.}
...
\begin{figure}[tb]
    ...
    \begin{minipage}[t]{\candwidth}
        \candpanelin{candidates_b_pinch1}{Pinch round $1$. The candidates of round $2$, on the levels of the slices just refined.}{fig:cand_pinch1}
        \\[4pt]
        \candpanelin{candidates_c_pinch2}{Pinch round $2$. Its own candidates, from which round $3$ detects nothing.}{fig:cand_pinch2}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{\candwidth}
        \candpanelin{candidates_d_conf1}{Confinement sweep $1$. The candidates of sweep $2$, the children of what it refined.}{fig:cand_conf1}
        \\[4pt]
        \candpanelin{candidates_e_conf2}{Confinement sweep $2$, one level finer, after which no leaf face is confined.}{fig:cand_conf2}
    \end{minipage}
    \hfill
    ...
    \caption{\robert{One padding cycle on a valence-$10$ ring mesh, read left to right. The two middle columns are the two inner iterations of Section~\ref{subsec:padding}, their rounds stacked. Each panel carries the candidate set of its own substep and of no other, the vertices of~\eqref{eq:pinch_candidates} in the pinch column and the faces of~\eqref{eq:confined_candidates} in the confinement column, coloured by the level they are candidates on. Both iterations need their second round. Round $1$ repairs the pinch of the input and opens a new one elsewhere, and sweep $1$ leaves children that are confined in turn, which is Lemma~\ref{lemma:confined_children}. Defects are not marked. Each is read off what the next panel refines.}}
    \label{fig:candidate_sets}
\end{figure}

\robert{The two round counts $q^{\mathrm{pinch}}$ of Corollary~\ref{lemma:pinch_spent} and $q^{\mathrm{confined}}$ above are the counts of the two inner iterations. They are not part of the count $q$ of productive $\textsc{Pad}$ passes of Section~\ref{subsec:termination}, which counts the passes driven by the links between the substeps alone.}
...
observation.
\robert{The first two factors of $\textsc{Pad}$ in~\eqref{eq:pad_operator} are themselves iterations, so $q$ counts the passes driven by the links between the substeps and nothing else. The counts $q^{\mathrm{pinch}}$ of Corollary~\ref{lemma:pinch_spent} and $q^{\mathrm{confined}}$ of Corollary~\ref{cor:confinement_rounds} are the two inner counts and are not part of $q$. This is how the implementation has always run, so the counts reported in Section~\ref{sec:numerical_experiments} are the ones this definition produces.}
...
\guidance{The same counting argument bounds the rounds of the two inner iterations, which is how Corollary~\ref{lemma:pinch_spent} and Corollary~\ref{cor:confinement_rounds} could have been proved; both give something sharper instead, a spent vertex-level pair and a strictly increasing level.}
...
\begin{remark}[\robert{Seeding one substep from another}]
    ...
    \robert{The chain of the next remark travels through a set this remark makes explicit. No substep has to start from the whole domain sequence. The initial candidate sets of Section~\ref{subsec:padding} are stated as $\V(\domseq[\L]^{\mathrm{refine}})$ and $\leafMesh(\domseq[\L]^{\mathrm{pinch}})$ for readability, and the results already proved shrink both. Three of them compose. Each substep discharges its own predicate, as recorded above. Lemma~\ref{lemma:pad_locality} says where a quadrisection can move a predicate. Lemma~\ref{lemma:new_vertices} settles the vertices a quadrisection creates, and the remark after it settles the faces.
    ...
    \begin{equation}
        ...
        \F^{\mathrm{cand},0} \; = \bigsqcup\limits_{\l = \lzero}^{\L-1} \Big( \FVl{\l} \big( \V_\l(\simplexsubset{2}{\l}) \big) \; \cup \; \Aop{}{\l}{\l+1} \big( \simplexsubset{2}{\l} \big) \Big).
    \end{equation}
    The children are not optional here. Clause~$(ii)$ of Lemma~\ref{lemma:pad_locality} speaks only about faces that were already leaf faces, and a child of a repaired class star can be confined by the same burial that Corollary~\ref{cor:confinement_rounds} counts.

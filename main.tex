...
\begin{lemma}[The graded vertex quadrisection is the least-refinement homogenisation]
    ...
    Order the domain sequences in $\alldomseqs_\L$ of $\T_\L$ by inclusion of their regions on every level. Let $\domseq[\L] \in \alldomseqs_\L$ and let $\markedVertexClasses$ and $\targetlevel$ be as in Definition~\ref{def:graded_vertex_quadrisection}. Then
    \begin{equation}
        \label{eq:graded_quadrisection_minimal}
        \quadriV \big( \domseq[\L], \, \markedVertexClasses, \, \targetlevel \big) \; = \; \min \, \Big\{ \, \domseq[\L]' \, \supseteq \, \domseq[\L] \;\; \colon \;\; \lmin \big( \FVleaf{}(\vertexClass{\coarseIdxone}) \big) \, \geq \, \targetlevel_\coarseIdxone \;\text{ read on }\; \domseq[\L]' \;\text{ for every }\; \vertexClass{\coarseIdxone} \in \markedVertexClasses \Big\},
    \end{equation}
    the coarsest refinement of $\domseq[\L]$ that homogenises every marked class to its target level.
\end{lemma}
\begin{proof}
    \robert{Let $\domseq[\L]' \supseteq \domseq[\L]$ with regions $\M'_\l$ satisfy the constraint, let $\vertexClass{\coarseIdxone} \in \markedVertexClasses$ and let $\l < \targetlevel_\coarseIdxone$. Every face of the mesh star $\FVl{\L}(\vertex{\L}{\coarseIdxone})$ is covered in $\domseq[\L]'$ by a leaf face on a level $\geq \targetlevel_\coarseIdxone > \l$, so its level-$\l$ ancestor is refined beyond level $\l$ and hence lies in $\ancestorPatch{\M'_{\l+1}}$. Those ancestors are exactly the class star $\FVanc{\l}(\vertexClass{\coarseIdxone})$ by Definition~\ref{def:star_ancestry}, so $\Aop{}{\l}{\l+1}(\FVanc{\l}(\vertexClass{\coarseIdxone})) \subseteq \M'_{\l+1}$. Taking the union over the marked classes and over the levels below their targets, and using $\M_{\l+1} \subseteq \M'_{\l+1}$, gives $\quadriV(\domseq[\L], \markedVertexClasses, \targetlevel) \subseteq \domseq[\L]'$ on every level by~\eqref{eq:graded_vertex_quadrisection}. The operator itself lies in the set, since Corollary~\ref{cor:graded_quadrisection_leafstar} gives $\lmin' \geq \max(\lmin, \targetlevel_\coarseIdxone)$ for every marked class, so it is the minimum.}
\end{proof}

The lemma is what rules out the wider neighbourhoods one might reach for instead. Refining the class star from level $\lzero$ up to $\l$ at every marked class would also homogenise, and would be strictly larger whenever some class already carries part of its star beyond $\lzero$. The levels below the birth level of a class are not a wider choice, however, but a forced one. A face enters a region only together with its ancestors, so reaching the class star on level $\l$ at all requires the class stars on the coarser levels, and Definition~\ref{def:star_ancestry} is what names that forced chain where the class has no representative to hang it on.

\smallskip

The graded vertex quadrisection operator is not evaluated through~\eqref{eq:graded_vertex_quadrisection} in practice. The implementation never forms a class star and works with the leaf star instead, one class and one level at a time.


\vspace{20mm}
\hrule

\todo{Does that make sense though? Isn't the closed form easier to implement? No re-computation, just adding the class stars level by level? That could mean that everything below the hrule can be cut.}

\begin{remark}[\robert{How the implementation evaluates $\quadriV$}]
    \label{rem:graded_quadrisection_algorithm}
    \robert{Let $\vertexClass{\coarseIdxone} \in \vertexClasses(\domseq[\L])$ be a single marked class with target level $\targetlevel_\coarseIdxone$, and let $\lmin = \lmin(\FVleaf{}(\vertexClass{\coarseIdxone}))$ be read on the input $\domseq[\L]$. The implementation grades the leaf star from $\lmin$ to $\targetlevel_\coarseIdxone$ by the iterative refinement algorithm}
    \begin{enumerate}
        \item initialise $\domseq[\L,\lmin] \coloneq \domseq[\L]$,
        \item iterate \robert{$\domseq[\L, \lone + 1] = \quadriF \big( \domseq[\L, \lone], \, \FVleaf{\lone}(\vertexClass{\coarseIdxone}) \big)$}, refining the coarsest slice of the leaf star read on $\domseq[\L, \lone]$, for $\lone \in \{ \lmin, \dots, \targetlevel_\coarseIdxone \! - \! 1 \}$,
        \item \robert{return $\domseq[\L, \targetlevel_\coarseIdxone]$, and return $\domseq[\L]$ unchanged if $\lmin \geq \targetlevel_\coarseIdxone$.}
    \end{enumerate}
    Note that step 2 recomputes the leaf star on every $\domseq[\L,\lone]$ that resulted from refining its coarsest faces on the previous step. This ensures that the refined children of exactly these faces are picked up and refined again until the target level is reached. Figure~\ref{fig:refine_interface} displays the nested refinement produced by graded quadrisection to the leaf stars of two interface vertices. The iterations up to $\targetlevel_\coarseIdxone = \lmax \!+\! 1$ bring the neighbourhood up to the finest level the leaf star already carries, and the last iteration refines every leaf face that contains the vertex one level further.
\end{remark}

\todo{can probably go once we have changed the algorithm above to be honest, seems more complicated to keep it.}

\begin{lemma}[\robert{The algorithm evaluates the operator}]
    \label{lemma:graded_quadrisection_closed_form}
    \robert{Let $\domseq[\L] \in \alldomseqs$, let $\vertexClass{\coarseIdxone} \in \vertexClasses(\domseq[\L])$ and let $\targetlevel$ be a target level map. The algorithm of Remark~\ref{rem:graded_quadrisection_algorithm} returns $\quadriV(\domseq[\L], \{ \vertexClass{\coarseIdxone} \}, \targetlevel)$.}
\end{lemma}
\begin{proof}
    \robert{Let $\vertex{\L}{\coarseIdxone}$ be the representative of $\vertexClass{\coarseIdxone}$ on level $\L$ as in Definition~\ref{def:vertex_class}, and for a level $\lone$ write $\targetlevel^{\lone}$ for the target level map that sends $\vertexClass{\coarseIdxone}$ to $\lone$, so that $\targetlevel = \targetlevel^{\targetlevel_\coarseIdxone}$ as far as this class is concerned. If $\lmin \geq \targetlevel_\coarseIdxone$, the algorithm returns $\domseq[\L]$ and the operator is the identity by Corollary~\ref{cor:graded_quadrisection_composition}.

    Let now $\lmin < \targetlevel_\coarseIdxone$ and let $\M^{(\lone)}$ denote the regions of the intermediate domain sequence $\domseq[\L, \lone]$ of the algorithm. We show by induction over $\lone \in \{ \lmin, \, \dots, \, \targetlevel_\coarseIdxone \}$ that $\domseq[\L, \lone] = \quadriV(\domseq[\L], \{ \vertexClass{\coarseIdxone} \}, \targetlevel^{\lone})$. \tobecut{and that the leaf star of $\vertexClass{\coarseIdxone}$ read on $\domseq[\L, \lone]$ has coarsest level $\lone$.} \robert{The leaf star of $\vertexClass{\coarseIdxone}$ read on such a $\domseq[\L, \lone]$ then has coarsest level $\max(\lmin, \lone) = \lone$ by Corollary~\ref{cor:graded_quadrisection_leafstar}, which is what the step below reads off it, so this does not have to be carried as a second claim.}

    This holds at $\lone = \lmin$, since the algorithm initialises $\domseq[\L, \lmin] = \domseq[\L]$ and $\quadriV(\domseq[\L], \{ \vertexClass{\coarseIdxone} \}, \targetlevel^{\lmin}) = \domseq[\L]$ by Corollary~\ref{cor:graded_quadrisection_composition}.

    Now take the step from $\lone$ to $\lone \! + \! 1$ with $\lone < \targetlevel_\coarseIdxone$. We first check that $\FVanc{\lone}(\vertexClass{\coarseIdxone}) \subseteq \M^{(\lone)}_\lone$. For $\lone = \lzero$ this is immediate, since $\M_\lzero = \F_\lzero(\T_\lzero)$ contains every level-$\lzero$ face. For $\lone > \lzero$ read~\eqref{eq:class_star_children} on $\domseq[\L, \lone]$ at level $\lone \! - \! 1$. That reading is permitted because the leaf star of $\vertexClass{\coarseIdxone}$ read there has coarsest level $\lone$ and so $\lone \! - \! 1$ lies below it. It places the children of $\FVanc{\lone-1}(\vertexClass{\coarseIdxone})$ in $\M^{(\lone)}_\lone$, and~\eqref{eq:star_ancestry_nested} places $\FVanc{\lone}(\vertexClass{\coarseIdxone})$ among those children. The split~\eqref{eq:leaf_ancestor_split} of the region into leaf faces and ancestor patch therefore applies to the star ancestry and gives
    \begin{equation}
        \label{eq:ancestry_splits}
        \FVanc{\lone}(\vertexClass{\coarseIdxone}) \; = \; \FVleaf{\lone}(\vertexClass{\coarseIdxone}) \; \sqcup \; \Big( \FVanc{\lone}(\vertexClass{\coarseIdxone}) \cap \ancestorPatch{\M^{(\lone)}_{\lone+1}} \Big),
    \end{equation}
    where the leaf star is read on $\domseq[\L, \lone]$. Indeed, a face of $\FVanc{\lone}(\vertexClass{\coarseIdxone})$ that is a leaf face on level $\lone$ covers a face of $\FVl{\L}(\vertex{\L}{\coarseIdxone})$ and hence lies in $\FVleaf{\lone}(\vertexClass{\coarseIdxone})$, and conversely by~\eqref{eq:leaf_star_in_ancestry}. The leaf star of $\vertexClass{\coarseIdxone}$ read on $\domseq[\L, \lone]$ has coarsest level $\lone$, so step 2 refines $\FVleaf{\lone}(\vertexClass{\coarseIdxone})$ and adds $\Aop{}{\lone}{\lone+1}(\FVleaf{\lone}(\vertexClass{\coarseIdxone}))$ to $\M^{(\lone)}_{\lone+1}$. The children of the second part of~\eqref{eq:ancestry_splits} already lie in $\M^{(\lone)}_{\lone+1}$, whence
    \begin{equation}
        \M^{(\lone+1)}_{\lone+1} \; = \; \M^{(\lone)}_{\lone+1} \; \cup \; \Aop{}{\lone}{\lone+1} \big( \FVanc{\lone}(\vertexClass{\coarseIdxone}) \big)
    \end{equation}
    and no other region changes, which by Definition~\ref{def:graded_vertex_quadrisection} is $\quadriV(\domseq[\L], \{ \vertexClass{\coarseIdxone} \}, \targetlevel^{\lone+1})$. \tobecut{Finally, every face of $\FVl{\L}(\vertex{\L}{\coarseIdxone})$ has its level-$\lone$ ancestor in $\FVanc{\lone}(\vertexClass{\coarseIdxone})$, and that ancestor is now refined beyond level $\lone$, so no face of the mesh star is covered on level $\lone$ or coarser and the leaf star of $\vertexClass{\coarseIdxone}$ read on $\domseq[\L, \lone \! + \! 1]$ has coarsest level $\lone \! + \! 1$.} The case $\lone = \targetlevel_\coarseIdxone$ of the induction is the assertion.}
\end{proof}

The quadrisection operators defined in this section are the basis upon which we build the algorithms in the adaptive loop in Section~\ref{subsec:adaptive_loop}. They enable nested refinement of the adaptive subdivision $k$-form spaces $\hierSDFspace{k}{\domseq}{\L}$ in the sense that
\begin{equation}
    \label{eq:quadrisection_monotone}
    \hierSDFspace{k}{\domseq}{\L} \subseteq \hierSDFspace{k}{\quadriF(\domseq[\L], \simplexsubset{2}{})}{} \qquad \text{and} \qquad \hierSDFspace{k}{\domseq}{\L} \subseteq \hierSDFspace{k}{\quadriV(\domseq[\L], \markedVertexClasses, \targetlevel)}{}.
\end{equation}
The second inclusion follows from the first, since $\quadriV$ is \tobecut{a composition of face quadrisections}\robert{a face quadrisection by Corollary~\ref{cor:graded_is_face_quadrisection}}. The reason the refinement produces \guidance{superspaces} is that the domain sequences satisfy $\M_\l \subseteq \M'_\l$, where $\M_\l$ and $\M'_\l$ are the regions of $\domseq[\L]$ and $\quadriF(\domseq[\L], \simplexsubset{2}{})$ or $\quadriV(\domseq[\L], \markedVertexClasses, \targetlevel)$, respectively\robert{, which both definitions make manifest because they only ever add faces to a region}. \cite{companion_paper} prove the nestedness of the adaptive $k$-form spaces given \guidance{the} nestedness of regions.
...
\begin{enumerate}[topsep=3pt, itemsep=-2pt]
    ...
    \begin{equation}
        \domseq[\L]^{p} = \quadriV \big( \domseq[\L]^{p-1}, \, \{ \vertexClass{\coarseIdxtwo_p} \}, \, \targetlevel \big),
    \end{equation}
    \vspace{-7mm}
...
The \textsc{Pad} step needs no machinery beyond the quadrisection operators $\quadriF$ \robert{and} $\quadriV$ of Section~\ref{subsec:refinement_operators}, since every padding substep is a quadrisection of either a vertex star or a set of confined faces.
...
% also covers $\quadriV$ in one application rather than factor by
...
\begin{lemma}[\robert{Locality of the padding predicates}]
    ...
    \robert{Let $\domseq[\L] \in \alldomseqs$, let $\simplexsubset{2}{}$ with slices $\simplexsubset{2}{\l}$ be a marked family for $\domseq[\L]$ in the sense of~\eqref{eq:marked_face_family}, and let $\domseq[\L]' \coloneq \quadriF(\domseq[\L], \simplexsubset{2}{})$ with refinement fans $\refFan{\lone}'$. \tobecut{The earlier version of this lemma asked for a set of leaf faces, $\simplexsubset{2}{\l} \subseteq \leafF{\l}(\domseq[\L])$ for every $\l < \L$, which the proof never uses.} Then
    \begin{equation}
...
\begin{equation}
    ...
    \domseq[\L]^p = \quadriV \big( \domseq[\L]^{p-1}, \, \{ \vertexClass{\coarseIdxtwo_p} \}, \, \targetlevel \big) \qquad \text{for all} \qquad (\vertex{\l}{\coarseIdxtwo})_p \in \V^{\mathrm{pinch}}(\domseq[\L]^{\mathrm{refine}}),
\end{equation}
where $p \in \{1, \, \dots, \, \vert \V^{\mathrm{pinch}}(\domseq[\L]^{\mathrm{refine}}) \vert \}$ is an enumeration of the pinch vertices and $\domseq[\L]^0 = \domseq[\L]^\mathrm{refine}$ is initialised to the domain sequence after the \textsc{Refine} step. \robert{Each step hands Definition~\ref{def:graded_vertex_quadrisection} the class $\vertexClass{\coarseIdxtwo_p}$ of the pinch vertex $(\vertex{\l}{\coarseIdxtwo})_p$, with the target level map given by $\targetlevel_\coarseIdxtwo = \l \! + \! 1$ read off the level at which the vertex was detected.}

\robert{The set $\V^{\mathrm{pinch}}$ is a set of vertices and not of classes, because the pinch test runs at every level. One class can therefore appear in it twice, once at a level $\l$ and once at a level $\lone > \l$. Written as a single application of $\quadriV$, the marked set is the set of classes occurring in $\V^{\mathrm{pinch}}$ and the target of such a class is the largest of the levels at which it was detected, plus one. Nothing is lost by taking the maximum, since the union in~\eqref{eq:graded_vertex_quadrisection} adds for a class marked at two targets exactly what the larger of the two adds on its own. The same holds for the grading step below, where a class can carry only one level span and the question does not arise.}
...
\robert{The candidate set~\eqref{eq:pinch_candidates} is exactly what Lemma~\ref{lemma:pad_locality} permits, and it differs from the one-ring of the repaired vertices in both directions. It is narrower, because a vertex enters as a candidate only on the level of the face that carried it and not on every level at which it exists. It is wider, because $\quadriV$ with target $\l \! + \! 1$ refines faces on all levels between $\lmin$ and $\l$, and a face on a level $\lone < \l$ of the leaf star of $\vertex{\l}{\coarseIdxtwo}$ need not carry $\vertex{\l}{\coarseIdxtwo}$ as a vertex at all: a vertex born on level $\l$ can lie in the interior of an edge of the leaf face covering it, and then the candidates are its level-$\lone$ neighbours, which lie further out than its level-$\l$ ring.}
...
\begin{lemma}[\robert{Pinch resolution spends its vertex}]
    ...
    \robert{Let $\vertex{\l}{\coarseIdxtwo}$ be a pinch of $\domseq[\L]$ on level $\l$ and let $\domseq[\L]' \coloneq \quadriV(\domseq[\L], \{ \vertexClass{\coarseIdxtwo} \}, \targetlevel)$ with $\targetlevel_\coarseIdxtwo = \l \! + \! 1$. Then the refinement fan of $\vertex{\l}{\coarseIdxtwo}$ in $\domseq[\L]'$ is its entire mesh star,
    \begin{equation}
...
% directly. Kept for cherry-picking.
    ...
    % SUPERSEDED (2026-08-25): the first two sentences are the first case of
    % Proposition~\ref{proposition:class_star_levels} read on the output. Kept for cherry-picking.
    \tobecut{Let $\lone \leq \l$ be a level at which $\vertex{\l}{\coarseIdxtwo}$ exists. By Definition~\ref{def:graded_vertex_quadrisection} with $\targetlevel = \l \! + \! 1$, the regions of $\domseq[\L]'$ contain $\Aop{}{\lone}{\lone+1}(\FVanc{\lone}(\vertex{\l}{\coarseIdxtwo}))$ on level $\lone \! + \! 1$, so every face of $\FVanc{\lone}(\vertex{\l}{\coarseIdxtwo})$ lies in $\ancestorPatch{\M'_{\lone+1}}$ and is refined beyond level $\lone$. That star ancestry is the mesh star $\FVl{\lone}(\vertex{\lone}{\coarseIdxtwo})$ by~\eqref{eq:star_ancestry_is_star}, so the refinement fan $\refFan{\lone}'(\vertex{\lone}{\coarseIdxtwo}) = \FVl{\lone}(\vertex{\lone}{\coarseIdxtwo}) \cap \ancestorPatch{\M'_{\lone+1}}$ of Definition~\ref{def:refinement_fan} is the whole mesh star, which is~\eqref{eq:pinch_spent}.}

    \robert{The leaf star of $\vertex{\l}{\coarseIdxtwo}$ read on $\domseq[\L]'$ has coarsest level $\lmin' = \l \! + \! 1$ by Corollary~\ref{cor:graded_quadrisection_leafstar}. The first case of~\eqref{eq:class_star_levels} read on $\domseq[\L]'$ therefore refines the whole class star beyond its own level on every level $\lone < \l \! + \! 1$, and that class star is the mesh star $\FVl{\lone}(\vertex{\lone}{\coarseIdxtwo})$ by~\eqref{eq:star_ancestry_is_star} at the levels $\lone$ at which the vertex exists, which is~\eqref{eq:pinch_spent}. The regions of a domain sequence only grow under the quadrisection operators, so~\eqref{eq:pinch_spent} survives every later refinement. A full refinement fan is a single arc and contains both extremes of a linear fan, so it is neither a pinch by Definition~\ref{def:pinch_vertex} nor a boundary pinch by Definition~\ref{def:boundary_pinch}. For the bound, every productive round repairs at least one pair of a vertex and a level, no pair is ever repaired twice by the first half, and pinches are detected only on levels $\l < \L$. The last round detects nothing and stops the iteration.}
\end{proof}
...
\begin{remark}[\robert{Why the repair homogenises the whole star}]
    ...
    \robert{The repair refines more than the defect requires. Arcs and gaps alternate around the mesh star, so their numbers agree, and Definition~\ref{def:pinch_vertex} asks for at least two gaps. Filling one gap merges the two arcs beside it, so a vertex with $k$ arcs needs $k \! - \! 1$ of its gaps filled and not all $k$, while the target $\l \! + \! 1$ fills every one. The gap left over would not have to be filled later either. Refinement only adds faces to a fan, so a gap can shrink from an end, split in two if it has at least three faces, or close and merge its arcs. A single gap of one or two faces has no interior face and can therefore never split, which leaves the vertex permanently free of pinches at that level. Homogenising is minimal neither among repairs nor among the repairs that last.

    We homogenise for three reasons. It is canonical, since no gap has to be selected and the result depends on the class and the target alone, which is what makes Corollary~\ref{cor:graded_quadrisection_composition} independent of the order. It costs no new operator and no new notion, because \textsc{Refine} and the grading step ask for homogenisation on their own account, whereas a partial repair would need a partial fan that Proposition~\ref{proposition:class_star_levels} does not cover. And it keeps the accounting short, since a full fan cannot be split, which is the whole of Lemma~\ref{lemma:pinch_spent} and hence of the bound~\eqref{eq:pinch_rounds}. A repair leaving one short gap would be spent as well by the argument above, so that bound is not lost in principle, but proving it would mean carrying gap lengths through the iteration. A partial repair would also refine a neighbourhood on one side only, working against the grading step below and its purpose of smoothing transitions. What is given up is one gap at one vertex on one level per repair, and Lemma~\ref{lemma:graded_quadrisection_minimal} bounds it from the other side, since the operator is the least refinement that homogenises at all.}
\end{remark}
...
\begin{enumerate}[topsep=3pt, itemsep=-2pt]
    ...
    \end{equation}
    \vspace{-7mm}
    \item refine them \tobecut{level by level }as in~\eqref{eq:confined_faces_resolved_subdiv_partition} to obtain $\domseq[\L]^{\mathrm{unconfined},k}$, and take the children of the faces just refined as the candidates of the next round,
    \begin{equation}
...
\begin{lemma}[\robert{Confinement padding confines only its own children}]
    ...
    \robert{Let $\domseq[\L] \in \alldomseqs$, let $\F^{\mathrm{confined}} \subseteq \leafMesh(\domseq[\L])$ be a set of leaf faces, every one of which is confined in $\domseq[\L]$, and let $\domseq[\L]' \coloneq \quadriF(\domseq[\L], \F^{\mathrm{confined}})$ be the result of refining them as in~\eqref{eq:confined_faces_resolved_subdiv_partition}. Then every leaf face of $\domseq[\L]'$ that is confined in $\domseq[\L]'$ and was not confined in $\domseq[\L]$ is a child of a face of $\F^{\mathrm{confined}}$.}
\end{lemma}
\begin{proof}
    ...
    \robert{Let $\face{\lone}{\fineIdxone}$ be a leaf face of $\domseq[\L]$ that is unconfined in $\domseq[\L]$ and is still a leaf face of $\domseq[\L]'$, and let $\vertex{\lone}{\coarseIdxtwo}$ be one of its vertices that is refinement-exterior on level $\lone$. Then $\vertex{\lone}{\coarseIdxtwo}$ is a vertex of no face of the slice $\F^{\mathrm{confined}}_\lone$. Such a face is confined and therefore has no refinement-exterior vertex on level $\lone$ by Definition~\ref{def:confined_face}, whereas the fan of $\vertex{\lone}{\coarseIdxtwo}$ is empty there. Lemma~\ref{lemma:pad_locality} therefore leaves $\refFan{\lone}(\vertex{\lone}{\coarseIdxtwo})$ unchanged, the vertex stays refinement-exterior and $\face{\lone}{\fineIdxone}$ stays unconfined. The only faces of $\domseq[\L]'$ that are not faces of $\domseq[\L]$ are the children of $\F^{\mathrm{confined}}$.}
\end{proof}

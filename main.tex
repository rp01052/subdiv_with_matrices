...
% family newly refined; \collectF is the face counterpart (children of the
% refined faces, by lemma:confined_children). Gamma is free; \partial carries
...
\guidance{PROTOTYPE (2026-08-26). The three items below are the reach argument. They are
proved but not yet folded into Section~\ref{subsec:termination}; the intended use is to replace
the phrase ``close enough to each other'' in Remark~\ref{rem:padding_chain} by the condition of
Lemma~\ref{lemma:class_reach}, and to state through Remark~\ref{rem:class_reach} why the
localisation half of Problem~\ref{prob:padding_passes} is the hard one.}
...
The \textsc{Pad} step needs no machinery beyond the quadrisection operators $\quadriF$ and $\quadriV$ of Section~\ref{subsec:refinement_operators}, since every padding substep is a quadrisection of either a vertex star or a set of confined faces. The \textsc{Pad} step is also agnostic to whether the \textsc{Refine} step uses face-based or vertex-star based refinement.

The \textsc{Pad} step has three substeps. The first two, pinch resolution and confinement padding, have the same structure. A \emph{candidate set} of vertices or of leaf faces is tested against the substep's predicate on a fixed input domain sequence. Every candidate that fires is repaired, and the candidates of the next round are read off the marked family $\simplexsubset{2}{}$ that the repair has actually refined. Testing the whole candidate set before repairing any of it makes a round independent of the order in which its repairs are applied. What makes the restricted candidate set of the next round sufficient is that a quadrisection can change a predicate on level $\l$ only at the vertices of the slice $\simplexsubset{2}{\l}$.

\begin{lemma}[Locality of the padding predicates]
    ...
    \begin{equation}
        ...
        \refFan{\l}' \big( \vertex{\l}{\coarseIdxone} \big) \; = \; \refFan{\l} \big( \vertex{\l}{\coarseIdxone} \big) \qquad \text{for all} \qquad \vertex{\l}{\coarseIdxone} \in \V_\l(\T_\l) \setminus \V_\l(\simplexsubset{2}{\l})
    \end{equation}
    holds on every level $\l \in \{ \lzero, \ldots, \L\}$ . Consequently,
    \begin{enumerate}[topsep=3pt, itemsep=-2pt]
        \item[$(i)$] a vertex that is neither a pinch nor a boundary pinch of $\domseq[\L]$ on level $\l$ is neither one of $\domseq[\L]'$ on level $\l$, unless it is a vertex of a face of $\simplexsubset{2}{\l}$, and
        \item[$(ii)$] a leaf face of $\domseq[\L]$ on level $\l$ that is unconfined in $\domseq[\L]$ and is still a leaf face of $\domseq[\L]'$ is unconfined in $\domseq[\L]'$, unless it shares a vertex with a face of $\simplexsubset{2}{\l}$.
    \end{enumerate}
    Every level is therefore decided by its own slice alone. On level $\l$ the pinch predicate of Definition~\ref{def:pinch_vertex} and the boundary pinch predicate of Definition~\ref{def:boundary_pinch} can change only at the vertices $\vertex{\l}{\coarseIdxone} \in \V_\l(\simplexsubset{2}{\l})$. The confinement predicate of Definition~\ref{def:confined_face} can change only at the leaf faces of the one-ring $\FVl{\l} \big( \V_\l(\simplexsubset{2}{\l}) \big)$. A slice $\simplexsubset{2}{\lone}$ on a level $\lone \neq \l$ changes neither of the two sets.
\end{lemma}
\begin{proof}
    By Definition~\ref{def:face_quadrisection_operator}, $\quadriF(\domseq[\L], \simplexsubset{2}{})$ enlarges the region $\M_{\l+1}$ by $\Aop{}{\l}{\l+1}(\simplexsubset{2}{\l})$ on every level, so it enlarges the ancestor patch $\ancestorPatch{\M_{\l+1}}$ by exactly the slice $\simplexsubset{2}{\l}$ and by nothing else, as computed in~\eqref{eq:face_quadrisection_nesting}. The refinement fan $\refFan{\l}(\vertex{\l}{\coarseIdxone}) = \FVl{\l}(\vertex{\l}{\coarseIdxone}) \cap \ancestorPatch{\M_{\l+1}}$ of Definition~\ref{def:refinement_fan} can therefore only change at a vertex whose mesh star on level $\l$ meets $\simplexsubset{2}{\l}$, that is at a vertex of a face of $\simplexsubset{2}{\l}$, which is~\eqref{eq:pad_locality}. Note that the levels do not interact. The slice $\simplexsubset{2}{\l}$ alone decides what changes on level $\l$, whatever the other slices contain. Both consequences follow because the three predicates are read off the refinement fan alone. The pinch of Definition~\ref{def:pinch_vertex} and the boundary pinch of Definition~\ref{def:boundary_pinch} count its arcs, and the confined face of Definition~\ref{def:confined_face} asks whether one of its vertices has an empty fan.
\end{proof}

The lemma is stated on the vertices the domain sequence already carries. The quadrisection also adds new vertices to the domain sequence $\domseq[\L]'$, and the next lemma shows that we do not need to include them in the candidate sets.

\begin{lemma}[New vertices need no separate test]
    \label{lemma:new_vertices}
    Let $\domseq[\L] \in \alldomseqs_\L$, let $\simplexsubset{2}{}$ be a marked family for $\domseq[\L]$ in the sense of~\eqref{eq:marked_face_family}, and let $\domseq[\L]' \coloneq \quadriF(\domseq[\L], \simplexsubset{2}{})$ with regions $\M'_\l$ and refinement fans $\refFan{\l}'$. Let
    \begin{equation}
        \label{eq:new_vertex}
        \vertex{\l}{\coarseIdxone} \; \in \; \V_\l \big( \M'_\l \big) \, \setminus \, \V_\l \big( \M_\l \big)
    \end{equation}
    be a vertex that the quadrisection adds to the domain sequence on level $\l$. Then on every level $\lone \geq \l$ one of the following two holds:
    \begin{enumerate}[topsep=3pt, itemsep=-2pt]
        \item[$(i)$] The vertex is one of the slice on that level, $\vertex{\l}{\coarseIdxone} \in \V_\lone(\simplexsubset{2}{\lone})$, or
        \item[$(ii)$] The fan is empty, $\refFan{\lone}'(\vertex{\l}{\coarseIdxone}) = \varnothing$, so $\vertex{\l}{\coarseIdxone}$ is refinement-exterior at level $\lone$ in $\domseq[\L]'$ and is neither a pinch nor a boundary pinch there.
    \end{enumerate}
    Note that $\vertex{\l}{\coarseIdxone}$ is new to the domain sequence and not to the mesh. It is a vertex of $\T_\lone$ for every $\lone \geq \l$ throughout, and its mesh star $\FVl{\lone}(\vertex{\l}{\coarseIdxone})$ is defined before the quadrisection as well.
\end{lemma}
\begin{proof}
    \robert{We first record that membership in the regions descends. Let $\lone > \l$ and $\vertex{\lone}{\coarseIdxone} = \Aop{}{\l}{\lone}(\vertex{\l}{\coarseIdxone})$. Suppose $\vertex{\lone}{\coarseIdxone} \in \V_\lone(\M_\lone)$. The region $\M_\lone$ is quadrisection-closed, so a face $F = \face{\lone}{\fineIdxone} \in \M_\lone$ carrying $\vertex{\lone}{\coarseIdxone}$ has a parent $P = \anc{\l}(\face{\lone}{\fineIdxone}) \in \ancestorPatch{\M_\lone} \subseteq \M_{\lone-1}$ by Definition~\ref{def:domseq}. The vertices of the four children of $P$ are the three vertices of $P$ and the three midpoints of its edges, and those midpoints are born on level $\lone$. The vertex $\vertex{\l}{\coarseIdxone}$ exists already on level $\l \leq \lone \! - \! 1$, so it is one of the three vertices of $P$ and hence $\vertex{\l}{\coarseIdxone} \in \V_{\lone-1}(\M_{\lone-1})$. Iterating down to level $\l$ contradicts~\eqref{eq:new_vertex}. So $\vertex{\l}{\coarseIdxone} \notin \V_\lone(\M_\lone)$ on every level $\lone \geq \l$.

    Now suppose $(i)$ fails on level $\lone$ and let $F \in \FVl{\lone}(\vertex{\l}{\coarseIdxone}) \cap \ancestorPatch{\M'_{\lone+1}}$. By~\eqref{eq:face_quadrisection_nesting} the quadrisection enlarges the ancestor patch on level $\lone$ by exactly the slice, so $F \in \ancestorPatch{\M_{\lone+1}}$ or $F \in \simplexsubset{2}{\lone}$. The second is excluded, since $F$ carries $\vertex{\l}{\coarseIdxone}$ and $(i)$ fails. The first gives $F \in \M_\lone$ by Definition~\ref{def:domseq} and hence $\vertex{\l}{\coarseIdxone} \in \V_\lone(\M_\lone)$, which the first paragraph excludes. So the intersection is empty, which is the refinement fan of Definition~\ref{def:refinement_fan} and hence $(ii)$. An empty fan carries no refinement arc, so neither Definition~\ref{def:pinch_vertex} nor Definition~\ref{def:boundary_pinch} can fire at $\vertex{\l}{\coarseIdxone}$ on level $\lone$.}
\end{proof}

Two remarks on the reach of the two lemmas. First, neither of them says anything about the faces that $\quadriF$ creates. The children of $\simplexsubset{2}{\l}$ are new leaf faces on level $\l\!+\!1$ and have to be tested, which is what the candidate sets below do. The new vertices are settled by Lemma~\ref{lemma:new_vertices} instead. Its case $(i)$ puts the vertex into the candidate set of the next round on its own, and its case $(ii)$ leaves nothing to test.

Second, the lemma covers the graded vertex quadrisection in a single application and not one level at a time. By Corollary~\ref{cor:graded_is_face_quadrisection} the operator $\quadriV(\cdot, \markedVertexClasses, \targetlevel)$ is a face quadrisection on the family~\eqref{eq:graded_marked_family} of class stars, so reading that family as the slice gives the statement. On level $\l$ the slice is the union of the class stars $\FVanc{\l}(\vertexClass{\coarseIdxone})$ over the marked classes with $\targetlevel_\coarseIdxone > \l$, and the predicates that can have changed on level $\l$ are those of its vertices.

\robert{The proof of Lemma~\ref{lemma:pad_locality} gives a little more than the statement. It enlarges the ancestor patch by the slice, so a face of $\simplexsubset{2}{\l}$ that already lies in $\ancestorPatch{\M_{\l+1}}$ enlarges nothing and leaves every refinement fan alone. Only the faces a quadrisection refines for the first time can move a predicate. Both substeps read their next candidates off exactly those faces, so we name the two maps once.}

\begin{definition}[\robert{Candidate collection}]
    \label{def:candidate_collection}
    \robert{Let $\domseq[\L] \in \alldomseqs_\L$ with regions $\M_\l$ and let $\simplexsubset{2}{}$ with slices $\simplexsubset{2}{\l}$ be a marked family for $\domseq[\L]$ in the sense of~\eqref{eq:marked_face_family}. \emph{Candidate collection} is the map
    \begin{equation}
    ...
    \end{equation}
    onto the vertices of the faces that $\quadriF(\domseq[\L], \simplexsubset{2}{})$ refines for the first time, each read at the level of its face, and the map
    \begin{equation}
        \label{eq:candidate_collection_faces}
        \collectF \big( \domseq[\L], \, \simplexsubset{2}{} \big) \; \coloneq \bigsqcup\limits_{\l = \lzero}^{\L-1} \Big( \FVl{\l} \big( \V_\l ( \simplexsubset{2}{\l} \setminus \ancestorPatch{\M_{\l+1}} ) \big) \; \cup \; \Aop{}{\l}{\l+1} \big( \simplexsubset{2}{\l} \setminus \ancestorPatch{\M_{\l+1}} \big) \Big)
    \end{equation}
    onto the one-ring of those vertices together with the children of those faces.}
\end{definition}

\robert{Each map collects what its predicate can reach. By clause $(i)$ of Lemma~\ref{lemma:pad_locality} no vertex outside~\eqref{eq:candidate_collection} carries a pinch or a boundary pinch of $\quadriF(\domseq[\L], \simplexsubset{2}{})$ that it did not already carry in $\domseq[\L]$. By Lemma~\ref{lemma:new_vertices} the vertices the quadrisection adds need no separate test. Case $(i)$ puts such a vertex into~\eqref{eq:candidate_collection} on its own and case $(ii)$ leaves nothing to test. Clause $(ii)$ of Lemma~\ref{lemma:pad_locality} gives the first term of~\eqref{eq:candidate_collection_faces}, and the second term carries the children, which are not covered by that clause because they are not leaf faces of $\domseq[\L]$. The superscripts are the ones $\quadriV$ and $\quadriF$ carry.}

\guidance{Two things to check before this is final. The subtraction of $\ancestorPatch{\M_{\l+1}}$ in~\eqref{eq:candidate_collection} is new. The old candidate set was $\V_\l(\simplexsubset{2}{\l})$ over the whole slice, which is strictly larger whenever a marked class star reaches into a region that is already refined, and that happens in every cycle after the first. Confirm that the implementation subtracts as well before the counts of Section~\ref{sec:numerical_experiments} are read against this. The other point is~\eqref{eq:candidate_collection_faces}. Its first term is dropped inside the confinement iteration by Lemma~\ref{lemma:confined_children}, so only~\eqref{eq:seed_confined} ever uses the map in full.}


\subsubsection{Resolving vertex pinches}

\robert{This substep detects pinches with Definitions~\ref{def:pinch_vertex} and~\ref{def:boundary_pinch} and repairs them with the graded vertex quadrisection $\quadriV$. We write $\V^{\mathrm{pinch}}(\domseq[\L]) \subseteq \V(\domseq[\L])$ for the vertices of a domain sequence that are a pinch or a boundary pinch, each at the levels at which they are one. The test runs at every level at which the vertex exists in the domain sequence and not only at its finest one, so one class can occur in $\V^{\mathrm{pinch}}$ at several levels at once.}

\robert{Detection does not return a set of vertices to repair. It returns the pair of arguments that the graded vertex quadrisection $\quadriV$ of Definition~\ref{def:graded_vertex_quadrisection} asks for.}

\begin{definition}[\robert{Pinch detection}]
    \label{def:pinch_detection}
    \robert{Let $\domseq[\L] \in \alldomseqs_\L$ and let $\V^{\mathrm{cand}} \subseteq \V(\domseq[\L])$ be a set of candidate vertices, each read at a level. \emph{Pinch detection} is the map
    \begin{equation}
...
\robert{The superscript is the one $\quadriV$ carries, since $\detectV$ produces exactly what $\quadriV$ consumes. Detection does two things at once. It squashes the candidate vertices down to their classes by~\eqref{eq:pinch_detection_classes}, because $\quadriV$ marks classes and not vertices. It then reads one level off the levels at which the class was caught by~\eqref{eq:pinch_detection_target}. A pinch at level $\l$ contributes the target $\l \! + \! 1$, and a class caught at several levels keeps the finest of them. A candidate class that is caught at no level is not marked at all.}

\robert{We write $\pinchClasses$ and not $\markedVertexClasses$ because the marked set of Definition~\ref{def:graded_vertex_quadrisection} is a generic argument. The \textsc{Refine} step of Section~\ref{subsec:mark_refine} marks by residual and the grading substep marks by level span, and neither has anything to do with pinches. The notation matches the sets the other two substeps detect, $\V^{\mathrm{pinch}}$ here and $\F^{\mathrm{confined}}$ in Section~\ref{subsubsec:confined_faces}, with the hat saying that these are classes rather than vertices.}
...
\robert{Table~\ref{tab:padding_interface_L4} reports $q^{\mathrm{pinch}}$ for every substep of the padding step for the mixed vector Laplacian discussed in Section~\ref{subsec:vector_laplace}. The counts stay small and do not grow with the mesh. The chains described in the previous paragraph are therefore short in practice on the meshes we run, even though we do not prove that.}
\end{remark}
...
\begin{remark}[Why this bound is a level count]
    \label{rem:confinement_cycles_bound}
    The bound~\eqref{eq:confinement_cycles} is in the number of levels and not in the size of the mesh. That is the contrast with Remark~\ref{rem:pinch_rounds_bound} for pinch resolution. Lemma~\ref{lemma:confined_children} confines the next cycle to the children of this one, so the coarsest defect level strictly increases and the iteration cannot chain sideways.
\end{remark}
...
\end{equation}
\robert{The first two factors are the iterations of the two paragraphs above and not single sweeps. The grading is one sweep per cycle, and the outer loop is what drives it to a fixed point.} We call a single application of the \textsc{Pad} operator a \emph{padding cycle}. Since pinch resolution can introduce confined faces, and confinement padding can introduce pinches, \robert{that link is the only reason to} iterate padding cycles to a fixed point\robert{, and we have to} show that it terminates at an admissible domain sequence. This is what the next Section~\ref{subsec:termination} studies.

\robert{With the two iterations in place, each of the two substeps of a padding cycle discharges its own predicate completely. $\domseq[\L]^{\mathrm{pinch}}$ carries no pinch and no boundary pinch vertex, and $\domseq[\L]^{\mathrm{confined}}$ carries no confined leaf face. What a padding cycle cannot discharge are the links \emph{between} the substeps, namely that a pinch resolution can confine a face and that the confinement padding can create a pinch. Those two links are the only reason the cycles have to be iterated at all, and the number of productive cycles counts exactly them.}
...
observation.
\robert{The first two factors of $\textsc{Pad}$ in~\eqref{eq:pad_operator} are themselves iterations, so $q$ counts the passes driven by the links between the substeps and nothing else. The counts $q^{\mathrm{pinch}}$ of Corollary~\ref{cor:pinch_rounds} and $q^{\mathrm{confined}}$ of Corollary~\ref{cor:confinement_cycles} are the two inner counts and are not part of $q$. This is how the implementation has always run, so the counts reported in Section~\ref{sec:numerical_experiments} are the ones this definition produces.}
...
% Corollary~\ref{cor:graded_quadrisection_composition}. Kept for cherry-picking.
    ...
    \robert{At a pinch vertex $\vertex{\l}{\coarseIdxone}$, two distinct refinement arcs of the refinement fan at level $\l$ are separated by at least one face by Definition~\ref{def:pinch_vertex}, \robert{so the fan is not the whole mesh star and the first case of~\eqref{eq:class_star_levels} gives $\lmin \leq \l < \l \! + \! 1$.} \tobecut{That face is not refined beyond level $\l$, so the leaf face covering it lies on a level $\leq \l$ and the leaf star of $\vertex{\l}{\coarseIdxone}$ satisfies $\lmin \leq \l < \l \! + \! 1$.} By Corollary~\ref{cor:graded_quadrisection_composition}, $\quadriVup{\l+1}$ is therefore not the identity at that vertex. The same argument covers a boundary pinch, whose unanchored arc is separated from both extremes of the linear refinement fan by such a face.} A confined face is detected on the leaf faces only, so $\quadriF$ is not the identity on it. A vertex of level span greater than $g$ carries leaf faces below $\lmaxIdx{\coarseIdxone} \! - \! g$, so $\quadriV$ with target $\targetlevel_\coarseIdxone = \lmaxIdx{\coarseIdxone} \! - \! g$ quadrisects at least one of them. Pinches are detected only at levels $\l < \L$ and the grading target never exceeds $\L \! - \! g$, so no repair refines beyond level $\L$.
\end{proof}

\robert{The lemma is stated for one firing of one predicate, so it applies to a single round of either inner iteration just as it does to a whole pass. It is therefore the productivity half of all three termination arguments, the two inner ones and the one below.}
...
\guidance{The same counting argument bounds the rounds of the two inner iterations, which is how Corollary~\ref{cor:pinch_rounds} and Corollary~\ref{cor:confinement_cycles} could have been proved; both give something sharper instead, a spent vertex-level pair and a strictly increasing level.}
...
\label{rem:padding_order}
    ...
    \robert{We run pinch resolution first because it refines the most, so that the faces it confines can still be repaired downstream in the same cycle, and the grading last because it has the least to do. By Section~\ref{subsec:refine} the \textsc{Refine} step lifts $\lmaxIdx{\coarseIdxtwo}$ by one level at a marked class and neither of the other two substeps lifts it at all, so a leaf star reaching the grading carries a span at most one level over $g$. There is no accumulation of wild level spans for a leading grading pass to flatten, which is what makes the grading an optional last step rather than a stage the other two depend on.}


    \tobecut{We run pinch resolution first because it refines the most, so that the defects it opens can still be repaired downstream in the same pass.}
...
\begin{remark}[\robert{Seeding one substep from another}]
    ...
    \robert{The chain of the next remark travels through a set this remark makes explicit. No substep has to start from the whole domain sequence. The initial candidate sets of Section~\ref{subsec:padding} are stated as $\V(\domseq[\L]^{\mathrm{refine}})$ and $\leafMesh(\domseq[\L]^{\mathrm{pinch}})$ for readability, and the results already proved shrink both. Three of them compose. Each substep discharges its own predicate, by Proposition~\ref{prop:pinch_discharged} and Proposition~\ref{prop:confined_discharged}. Lemma~\ref{lemma:pad_locality} says where a quadrisection can move a predicate. Lemma~\ref{lemma:new_vertices} settles the vertices a quadrisection creates, and the remark after it settles the faces.

    Write $\simplexsubset{2}{\l}$ for the union over the rounds of a substep of the level-$\l$ slices it refined. Two handoffs follow.

    From confinement to pinch. The sequence entering the confinement iteration carries no pinch on any level. So the pinch iteration of the next cycle may start from the vertices of what the confinement padding and the grading refined,
    \begin{equation}
        \label{eq:seed_pinch}
        \V^{\mathrm{cand},0} \; = \bigsqcup\limits_{\l = \lzero}^{\L-1} \V_\l \big( \simplexsubset{2}{\l} \big).
    \end{equation}
    The grading has to be counted in, because it is a graded vertex quadrisection and runs last, so it can reopen all three predicates. Note also that these are the faces \emph{refined} and not the faces added. A predicate moves on level $\l$ at the vertices of the level-$\l$ slice, whereas the children lie on level $\l\!+\!1$ and the vertices born with them are settled by Lemma~\ref{lemma:new_vertices}.

    From pinch to confinement. The sequence entering the pinch iteration carries no confined leaf face, by the previous cycle or by admissibility of the input to \textsc{Refine}. So the confinement iteration may start from the one-ring of the pinch slices together with their children,
    \begin{equation}
        \label{eq:seed_confined}
        \F^{\mathrm{cand},0} \; = \; \collectF \big( \domseq[\L], \, \simplexsubset{2}{} \big) .
    \end{equation}
    The second term of~\eqref{eq:candidate_collection_faces} is not optional here. Clause~$(ii)$ of Lemma~\ref{lemma:pad_locality} speaks only about faces that were already leaf faces, and a child of a repaired class star can be confined by the same burial that Corollary~\ref{cor:confinement_cycles} counts. Lemma~\ref{lemma:confined_children} does not apply, because the slice refined here is a class star and not a set of confined faces.

    Both seeds close the loop back to \textsc{Refine}. Its input is admissible, so the first pinch iteration of the first cycle may be seeded by~\eqref{eq:seed_pinch} read on the marked family of the \textsc{Refine} step. The \textsc{Pad} operator then never scans the domain sequence at all, and touches only what the step before it has refined.}
\end{remark}

\guidance{The implementation does not do this. Both substeps restart from the full scan, which is what Section~\ref{subsec:padding} describes and what the reported counts come from. The saving is real but not on the critical path, since the scan is linear in the mesh and the solve dominates. Measured on the valence-$10$ domain sequence of the candidate-set figure, once that figure lands and this becomes a \texttt{\textbackslash ref}, the seed~\eqref{eq:seed_pinch} carries $16$ vertices against the $629$ that the full initial set carries.}
...
\begin{remark}[Chaining of padding passes]
    ...
    \robert{Lemma~\ref{lemma:class_reach} makes ``close enough'' precise. A repair at one class can only open a defect at a class whose star it meets, and two stars meet at level $\l$ only if the two classes are born no earlier than $\l \! - \! 1$, by~\eqref{eq:meeting_level_bound} when they share a face and by~\eqref{eq:class_reach} when they share only a vertex. A chain therefore runs through classes of nearly equal birth level, and the one extra level of~\eqref{eq:class_reach} is available only between classes that were already adjacent one level coarser. Successive links of a chain are neighbours in this sense, so its length is bounded by how far one can walk through $\T_\lzero$ in such steps.}
\end{remark}

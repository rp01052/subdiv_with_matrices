...
\robert{The substep detects pinches with Definitions~\ref{def:pinch_vertex} and~\ref{def:boundary_pinch} and repairs them with the graded vertex quadrisection $\quadriV$. We write $\V^{\mathrm{pinch}}(\domseq[\L]) \subseteq \V(\domseq[\L])$ for the vertices of a domain sequence that are a pinch or a boundary pinch, each at the levels at which they are one. The test runs at every level at which the vertex exists in the domain sequence and not only at its finest one, so one class can occur in $\V^{\mathrm{pinch}}$ at several levels at once.}

\robert{The target level $\l \! + \! 1$ is what merges the arcs, and it is the least target that does. The refinement arcs at level $\l$ are separated by faces that are not refined beyond level $\l$. Grading the leaf star to $\l \! + \! 1$ refines every one of them, so the fan at level $\l$ becomes the whole mesh star. Quadrisecting that mesh star once, without the grading, does not suffice, because the separation between the arcs can be covered by a leaf face at a level well below $\l$, and one round then lifts it by one level only and leaves the arcs separated.}

\begin{lemma}[\robert{Pinch resolution spends its vertex}]
    \label{lemma:pinch_spent}
    \robert{Let $\domseq[\L]' \coloneq \quadriV(\domseq[\L], \markedVertexClasses, \targetlevel)$ and let $\vertexClass{\coarseIdxtwo} \in \markedVertexClasses$ be any marked class. Then the refinement fan of $\vertexClass{\coarseIdxtwo}$ in $\domseq[\L]'$ is its entire mesh star,
    \begin{equation}
        \label{eq:pinch_spent}
        \refFan{\lone}' \big( \vertex{\lone}{\coarseIdxtwo} \big) \; = \; \FVl{\lone} \big( \vertex{\lone}{\coarseIdxtwo} \big),
    \end{equation}
    on every level $\lone < \targetlevel_\coarseIdxtwo$ at which the vertex exists, and the same holds in every domain sequence obtained from $\domseq[\L]'$ by further quadrisection. In particular, $\vertex{\lone}{\coarseIdxtwo}$ is never again a pinch or a boundary pinch on such a level. Read on a pinch caught at level $\l$ with the target $\l \! + \! 1$, this is what closes that pinch and what settles every level below $\l$ at the same time.}
\end{lemma}
\begin{proof}
    \robert{The leaf star of $\vertexClass{\coarseIdxtwo}$ read on $\domseq[\L]'$ has coarsest level $\lmin' \geq \targetlevel_\coarseIdxtwo$ by Corollary~\ref{cor:graded_quadrisection_leafstar}, with equality when $\vertexClass{\coarseIdxtwo}$ is marked on its own and $\lmin'$ only larger when a neighbour's class star reaches it too. The first case of~\eqref{eq:class_star_levels} read on $\domseq[\L]'$ therefore refines the whole class star beyond its own level on every level $\lone < \targetlevel_\coarseIdxtwo$, and that class star is the mesh star $\FVl{\lone}(\vertex{\lone}{\coarseIdxtwo})$ by~\eqref{eq:star_ancestry_is_star} at the levels $\lone$ at which the vertex exists, which is~\eqref{eq:pinch_spent}. The regions of a domain sequence only grow under the quadrisection operators, so~\eqref{eq:pinch_spent} survives every later refinement. A full refinement fan is a single arc and contains both extremes of a linear fan, so it is neither a pinch by Definition~\ref{def:pinch_vertex} nor a boundary pinch by Definition~\ref{def:boundary_pinch}.}
\end{proof}

\begin{corollary}[\robert{A repaired class leaves the candidate set}]
    \label{cor:pinch_spent_candidates}
    \robert{In the situation of Lemma~\ref{lemma:pinch_spent}, no vertex
    \begin{equation}
        \label{eq:pinch_spent_vertices}
        \vertex{\lone}{\coarseIdxtwo} \quad \text{with} \quad \vertexClass{\coarseIdxtwo} \in \markedVertexClasses \quad \text{and} \quad \lone \, < \, \targetlevel_\coarseIdxtwo
    \end{equation}
    is a pinch or a boundary pinch of $\domseq[\L]'$, nor of any domain sequence obtained from $\domseq[\L]'$ by further quadrisection. Such a vertex therefore never has to be tested again, whatever is refined later and in whatever order.}
\end{corollary}
\begin{proof}
    \robert{By~\eqref{eq:pinch_spent} its refinement fan is the whole mesh star on every such level, and Lemma~\ref{lemma:pinch_spent} carries that through every later quadrisection. A full fan is a single arc and contains both extremes of a linear fan, so Definition~\ref{def:pinch_vertex} and Definition~\ref{def:boundary_pinch} both fail. Testing it can only ever return nothing.}
\end{proof}

\robert{Nothing here forbids $\vertexClass{\coarseIdxtwo}$ from becoming a pinch on a level $\lone \geq \targetlevel_\coarseIdxtwo$, and nothing should. By~\eqref{eq:graded_marked_family} the application has slices on the levels below its target only and says nothing about the levels above. A class spent up to its target re-enters at a finer level through the slice of a different class, exactly as it should, which is why~\eqref{eq:pinch_spent_vertices} names vertices and not classes.}

\robert{Detection therefore does not return a set of vertices to repair. It returns the target level map $\targetlevel$, one value per vertex class, which is the second argument $\quadriV$ asks for. A class caught at several levels is repaired at the finest of them, since Lemma~\ref{lemma:pinch_spent} settles every level below the target in one go. A class caught at none is given its own $\lminIdx{\coarseIdxone}$, which makes the map total and is inert, because Corollary~\ref{cor:graded_quadrisection_composition} makes $\quadriV$ the identity at a class with $\targetlevel_\coarseIdxone \leq \lminIdx{\coarseIdxone}$. That floor never overrides a detection, since a pinch at level $\l$ has a refinement fan that is a non-empty proper subset of the mesh star, so $\lminIdx{\coarseIdxone} \leq \l < \l \! + \! 1$ by Proposition~\ref{proposition:class_star_levels}.}

\robert{The repair is then a single application of $\quadriV$ and not an enumeration over the caught vertices. Corollary~\ref{cor:graded_quadrisection_composition} says the two agree, so nothing is given up and independence of any ordering comes for free.}

\robert{One such application does not leave a domain sequence free of pinches. A repair can open a pinch at a vertex that carried none, at the vertices Lemma~\ref{lemma:pad_locality} allows. The substep is therefore an iteration, and we state it over a candidate set from the start. Let $\V^{\mathrm{seed}} \subseteq \V(\domseq[\L]^{\mathrm{refine}})$ be a \emph{seed}.}

\begin{iteration}[\robert{Pinch resolution}]
\label{it:pinch_iteration}
\robert{\begin{enumerate}[topsep=3pt, itemsep=-2pt]
    \item Initialise $\domseq[\L]^{\mathrm{pinch},0} \coloneq \domseq[\L]^{\mathrm{refine}}$ and $\V^{\mathrm{cand},0} \coloneq \V^{\mathrm{seed}}$.
    \item In round $k \geq 1$, let $\markedVertexClasses^k$ be the classes occurring in $\V^{\mathrm{cand},k-1}$ and compute the target level map $\targetlevel^k$ on them, with the pinches read on the unmutated $\domseq[\L]^{\mathrm{pinch},k-1}$ and restricted to the candidates,
    \begin{equation}
        \label{eq:pinch_round_detection}
        \targetlevel^k_\coarseIdxone \; = \; \max \Big( \lminIdx{\coarseIdxone}, \; \max \big\{ \, \l \! + \! 1 \; \colon \; \vertex{\l}{\coarseIdxone} \in \V^{\mathrm{pinch}} \big( \domseq[\L]^{\mathrm{pinch},k-1} \big) \, \cap \, \V^{\mathrm{cand},k-1} \big\} \Big).
    \end{equation}
    \vspace{-7mm}
    \item Repair in one application,
    \begin{equation}
        \label{eq:pinch_round_repair}
        \domseq[\L]^{\mathrm{pinch},k} \; \coloneq \; \quadriV \big( \domseq[\L]^{\mathrm{pinch},k-1}, \, \markedVertexClasses^k, \, \targetlevel^k \big).
    \end{equation}
    \vspace{-7mm}
    \item Round $k$ is a quadrisection of the marked family $\simplexsubset{2}{k}$ that~\eqref{eq:graded_marked_family} builds from $\markedVertexClasses^k$ and $\targetlevel^k$, with slices $\simplexsubset{2}{k,\l}$. The candidates of the next round are its vertices, less what Corollary~\ref{cor:pinch_spent_candidates} has already spent,
    \begin{equation}
        \label{eq:pinch_candidates}
        \V^{\mathrm{cand},k} \coloneq \Big( \bigsqcup\limits_{\l = \lzero}^{\L-1} \V_\l \big( \simplexsubset{2}{k,\l} \big) \Big) \; \setminus \; \big\{ \vertex{\lone}{\coarseIdxone} \; \colon \; \vertexClass{\coarseIdxone} \in \markedVertexClasses^k \; \text{ and } \; \lone < \targetlevel^k_\coarseIdxone \big\} ,
    \end{equation}
    \vspace{-7mm}
    \item Stop at the first round in which $\targetlevel^k$ is inert on every class, that is $\targetlevel^k_\coarseIdxone \leq \lminIdx{\coarseIdxone}$ throughout, and set $\domseq[\L]^{\mathrm{pinch}} \coloneq \domseq[\L]^{\mathrm{pinch},k}$.
\end{enumerate}}
\end{iteration}

\robert{Every class the round repaired is spent up to its own target, permanently, so the subtraction in step $4$ strikes its vertices below that target rather than carrying them. By Corollary~\ref{cor:graded_quadrisection_composition} the stopping test of step $5$ says exactly that round $k$ is the identity, and by~\eqref{eq:pinch_round_detection} it says exactly that no candidate was caught. Detection and repair stay separate phases within a round, since $\targetlevel^k$ is computed on the whole candidate set before any of it is refined.}

\robert{The seed may always be taken to be everything, $\V^{\mathrm{seed}} = \V(\domseq[\L]^{\mathrm{refine}})$, which is every vertex on every level by~\eqref{eq:vertices_of_domseq}. That is what our implementation does and it is what the counts of Section~\ref{sec:numerical_experiments} come from. A smaller seed is enough whenever the domain sequence entering the substep is already known to be free of pinches outside some set, which is the case in every pass of the padding loop, and we pick that thread up in Remark~\ref{rem:substep_seeding} of Section~\ref{subsec:termination}.}

\begin{corollary}[\robert{The pinch iteration terminates}]
    \label{cor:pinch_rounds}
    \robert{The number $q^{\mathrm{pinch}}$ of productive rounds of the iteration above is bounded by the number of vertex-level pairs the domain sequence carries,
    \begin{equation}
...
\end{corollary}
\begin{proof}
    \robert{Every productive round catches at least one pair of a vertex and a level, no pair is ever caught twice by Lemma~\ref{lemma:pinch_spent}, and pinches are detected only on levels $\l < \L$. The last round catches nothing and stops the iteration by step $5$.}
\end{proof}

\robert{The candidate set~\eqref{eq:pinch_candidates} is exactly what Lemma~\ref{lemma:pad_locality} permits, and it differs from the one-ring of the repaired vertices in both directions. It is narrower, because a vertex enters only on the level $\l$ of the slice $\simplexsubset{2}{k,\l}$ containing it and not on every level at which it exists. It is wider, because $\quadriV$ with target $\l \! + \! 1$ refines faces on all levels between $\lmin$ and $\l$, and a face on a level $\lone < \l$ of the leaf star of $\vertex{\l}{\coarseIdxtwo}$ need not carry $\vertex{\l}{\coarseIdxtwo}$ as a vertex at all. A vertex born on level $\l$ can lie in the interior of an edge of the leaf face covering it, and then the candidates are its level-$\lone$ neighbours, which lie further out than its level-$\l$ ring.}

\robert{The target level $\targetlevel = \l\!+\!1$ is what merges the arcs.} The refinement arcs at level $\l$ are separated by faces that are not refined beyond \robert{level $\l$}. Grading the leaf star to $\l\!+\!1$ refines every one of them, so the fan at level $\l$ becomes the whole mesh star. \robert{Quadrisecting the mesh star once, without the grading,} does not suffice, because the separation between the refinement arcs can be covered by a leaf face at a level well below $\l$, in which case one round lifts it only by one level and leaves the arcs separated.

\robert{Nothing is lost by repairing a class at the finest level it was caught on. The union in~\eqref{eq:graded_vertex_quadrisection} adds, for a class marked at two targets, exactly what the larger of the two adds on its own.}
...
\robert{A round that detects nothing certifies more than its own emptiness. Every vertex whose refinement fan has changed since the previous detection is a candidate by Lemma~\ref{lemma:pad_locality}, and every vertex the round has added is a candidate or refinement-exterior by Lemma~\ref{lemma:new_vertices}. So an empty round proves that no vertex of $\domseq[\L]^{\mathrm{pinch}}$ is a pinch or a boundary pinch on any level. The substep therefore has an exact postcondition\robert{. The output} $\domseq[\L]^{\mathrm{pinch}}$ satisfies clauses~(ii) and~(iii) of Definition~\ref{def:subdiv_partition}.}
...
\begin{remark}[\robert{Why the repair homogenises the whole star}]
    ...
    We homogenise for three reasons, and each of them is a result already proved.
    ...
    It costs no new notion. \textsc{Refine} and the grading step ask for homogenisation on their own account, and a homogenised class has the closed-form leaf star~\eqref{eq:graded_quadrisection_leafstar} and falls under the three cases of Proposition~\ref{proposition:class_star_levels}. A partially filled fan has no such description, so a partial repair would need machinery that the rest of the section then could not use.

    It keeps the accounting short, and this is where it pays. A full fan is a single arc and cannot be split again, which is the whole of Lemma~\ref{lemma:pinch_spent}. Everything downstream is that one fact read twice. The round bound~\eqref{eq:pinch_rounds} is it counted over the iteration, and Corollary~\ref{cor:pinch_spent_candidates} is it read on the candidate set, where it is what lets a repaired class be struck from~\eqref{eq:pinch_candidates} permanently rather than carried and retested. A repair leaving one short gap would be spent as well, by the argument above, so neither is lost in principle. Proving them would mean carrying gap lengths through the iteration instead of a single target level.
...
\robert{The two round counts $q^{\mathrm{pinch}}$ of Lemma~\ref{lemma:pinch_spent} and $q^{\mathrm{confined}}$ above are the counts of the two inner iterations. They are not part of the count $q$ of productive $\textsc{Pad}$ passes of Section~\ref{subsec:termination}, which counts the passes driven by the links between the substeps alone.}
...
observation.
\robert{The first two factors of $\textsc{Pad}$ in~\eqref{eq:pad_operator} are themselves iterations, so $q$ counts the passes driven by the links between the substeps and nothing else. The counts $q^{\mathrm{pinch}}$ of Lemma~\ref{lemma:pinch_spent} and $q^{\mathrm{confined}}$ of Corollary~\ref{cor:confinement_rounds} are the two inner counts and are not part of $q$. This is how the implementation has always run, so the counts reported in Section~\ref{sec:numerical_experiments} are the ones this definition produces.}
...
\guidance{The same counting argument bounds the rounds of the two inner iterations, which is how Lemma~\ref{lemma:pinch_spent} and Corollary~\ref{cor:confinement_rounds} could have been proved; both give something sharper instead, a spent vertex-level pair and a strictly increasing level.}

...
% There is no \collectG for the level span. It was written and retired the same
% day: the span is the one predicate a newborn midpoint can fire, so the map would
% have had to read the CHILDREN rather than the slice, and the grading is optional
% enough that a scan of the classes is the honest cost. See the remark after
...
% already spent: \pinchClasses is \widehat{\V}^{pinch} and \pinchClasses[c] is
% \widehat{\V}^{pinch,c}. Writing \pinchClasses[c] would be a double superscript.
\makeatletter
...
Note that Lemma~\ref{lemma:pad_locality} also covers graded vertex quadrisection. By Corollary~\ref{cor:graded_is_face_quadrisection} the operator $\quadriV(\cdot, \markedVertexClasses, \targetlevel)$ is a face quadrisection on the family~\eqref{eq:graded_marked_family} of class stars, so reading that family as the slice makes the lemma applicable. On level $\l$ the slice is the union of the class stars $\FVanc{\l}(\vertexClass{\coarseIdxone})$ over the marked classes with $\targetlevel_\coarseIdxone > \l$, and the predicates that can have changed on level $\l$ are those of its vertices.
...
Pinch detection computes the arguments that the graded vertex quadrisection $\quadriV$ of Definition~\ref{def:graded_vertex_quadrisection} asks for, namely the vertex classes that contain pinches and a target level that repairs the finest pinch of each vertex class.
...
\begin{definition}[Pinch detection]
    ...
    \end{equation}
    onto the vertex classes caught among the candidates,
    \begin{equation}
        \label{eq:pinch_detection_classes}
        \pinchClasses \; \coloneq \; \big\{ \vertexClass{\coarseIdxone} \in \vertexClasses(\domseq[\L]) \; \colon \;\; \vertex{\l}{\coarseIdxone} \in \V^{\mathrm{cand}} \text{\; is a pinch of } \domseq[\L] \;\text{ for at least one level }\; \l \big\},
    \end{equation}
    and the target level map on them,
    \begin{equation}
        \label{eq:pinch_detection_target}
        \targetlevel_\coarseIdxone \; \coloneq \; \max \big\{ \, \l \! + \! 1 \; \colon \;\; \vertex{\l}{\coarseIdxone} \in \V^{\mathrm{cand}} \text{\; is a pinch of } \domseq[\L] \big\} .
    \end{equation}
    Here and below a \emph{pinch} means a pinch or a boundary pinch, that is, membership in $\V^{\mathrm{pinch}}(\domseq[\L])$. The set in~\eqref{eq:pinch_detection_target} is non-empty for every $\vertexClass{\coarseIdxone} \in \pinchClasses$ by~\eqref{eq:pinch_detection_classes}, so the maximum is always taken over at least one level.
\end{definition}

\robert{The repair homogenises every marked class to its target in the sense of~\eqref{eq:homogenisation_constraint}. Corollary~\ref{lemma:pinch_spent} then spends the vertex on every level below that target.}

\robert{One application does not leave a domain sequence free of pinches. A repair can open a pinch at a vertex that carried none, at the vertices Lemma~\ref{lemma:pad_locality} allows. The substep is therefore an iteration over a candidate set. A vertex $\vertex{\l}{\coarseIdxone}$ spent on a level $\l < \targetlevel_\coarseIdxone$ never returns to that set. Its class $\vertexClass{\coarseIdxone}$ can return on a level $\lone \geq \targetlevel_\coarseIdxone$, through the slice of a different class. Let $\V^{\mathrm{seed}} \subseteq \V(\domseq[\L]^{\mathrm{refine}})$ be a \emph{seed}.}

\begin{iteration}[Pinch resolution]
\label{it:pinch_iteration}
    The pinch resolution step is initialised with the domain sequence $\domseq[\L]^{\mathrm{pinch},0} \coloneq \domseq[\L]^{\mathrm{refine}}$ and candidate set $\V^{\mathrm{cand},0} \coloneq \V^{\mathrm{seed}}$. It then iterates the following \emph{pinch resolution cycle} for $c \in \{1, \, \ldots, \, q^{\mathrm{pinch}} \}$:
    \begin{enumerate}[topsep=3pt, itemsep=-2pt]
    \item Detect pinches for the candidate set $\V^{\mathrm{cand},c-1}$ of the previous cycle on the domain sequence $\domseq[\L]^{\mathrm{pinch},c-1}$ to obtain
    \begin{equation}
        \label{eq:pinch_round_detection}
        \big( \pinchClasses[c], \, \targetlevel^c \big) \; \coloneq \; \detectV \big( \domseq[\L]^{\mathrm{pinch},c-1}, \, \V^{\mathrm{cand},c-1} \big).
    \end{equation}
    \vspace{-7mm}
    \item Repair the detected pinches by homogenising every caught class to its target
    \begin{equation}
        \label{eq:pinch_round_repair}
        \domseq[\L]^{\mathrm{pinch},c} \; \coloneq \; \quadriV \big( \domseq[\L]^{\mathrm{pinch},c-1}, \, \pinchClasses[c], \, \targetlevel^c \big).
    \end{equation}
    \vspace{-7mm}
    \item Collect the candidates for the next cycle based on the marked family $\simplexsubset{2,c}{}$ that~\eqref{eq:graded_marked_family} builds from $\pinchClasses[c]$ and $\targetlevel^c$, less what Corollary~\ref{lemma:pinch_spent} has already spent,
    \begin{equation}
        \label{eq:pinch_candidates}
        \V^{\mathrm{cand},c} \coloneq \collectV \big( \domseq[\L]^{\mathrm{pinch},c-1}, \, \simplexsubset{2,c}{} \big) \; \setminus \; \big\{ \vertex{\l}{\coarseIdxone} \; \colon \; \vertexClass{\coarseIdxone} \in \pinchClasses[c] \; \text{ and } \; \l < \targetlevel^c_\coarseIdxone \big\} ,
    \end{equation}
\end{enumerate}
We repeat steps $1$ to $3$ until the pinch detection in~\eqref{eq:pinch_round_detection} detects no more pinches. We then set $\domseq[\L]^{\mathrm{pinch}} \coloneq \domseq[\L]^{\mathrm{pinch},c}$ and write $q^{\mathrm{pinch}} \coloneq c \! - \! 1$ for the number of cycles until the fixed point was reached.
\end{iteration}

The iteration stops when the pinch detection catches nothing, i.e. $\pinchClasses[c] = \varnothing$. This criterion is equivalent to three other ones. First, in implies an empty  marked family $\simplexsubset{2,c}{}$. An empty marked family adjoins nothing in~\eqref{eq:graded_vertex_quadrisection} and leaves the candidate collection~\eqref{eq:candidate_collection} with no candidates to collect, so $\V^{\mathrm{cand},c} = \varnothing$. Finally, the stopping rule above is also the fixed point $\domseq[\L]^{\mathrm{pinch},c} = \domseq[\L]^{\mathrm{pinch},c-1}$ of the iteration.

The seed $\V^{\mathrm{seed}}$ may always be taken to be all vertices $\V(\domseq[\L]^{\mathrm{refine}})$ of the input domain sequence. A smaller seed can be used whenever the domain sequence entering the substep is already known to be free of pinches outside a given vertex set, which is the case in every pass of the overarching padding loop in Section~\ref{subsec:termination}.

\begin{corollary}[The pinch iteration terminates]
    \label{cor:pinch_rounds}
    The number $q^{\mathrm{pinch}}$ of productive pinch resolution cycles of Iteration~\ref{it:pinch_iteration} above is bounded by the number of vertices the input domain sequence $\domseq[\L]^{\mathrm{refine}}$ with regions $\M^{\mathrm{pinch}}_\l$ carries below level $\L$, i.e.
    \begin{equation}
        \label{eq:pinch_rounds}
        q^{\mathrm{pinch}} \; \leq \; \sum\limits_{\l = \lzero}^{\L-1} \; \big\vert \, \V_\l(\M^{\mathrm{pinch}}_\l) \, \big\vert \, .
    \end{equation}
\end{corollary}
\begin{proof}
    Every productive round catches at least one pair of a vertex and a level, no pair is ever caught twice by Corollary~\ref{lemma:pinch_spent}, and pinches are detected only on levels $\l < \L$.
\end{proof}

\begin{proposition}[\robert{The pinch iteration discharges clauses (ii) and (iii)}]
    \label{prop:pinch_discharged}
    \robert{Let the seed cover the pinches of the input, $\V^{\mathrm{pinch}}(\domseq[\L]^{\mathrm{refine}}) \subseteq \V^{\mathrm{seed}}$. Then no vertex of the output $\domseq[\L]^{\mathrm{pinch}}$ of Iteration~\ref{it:pinch_iteration} is a pinch or a boundary pinch on any level, so $\domseq[\L]^{\mathrm{pinch}}$ satisfies clauses~(ii) and~(iii) of Definition~\ref{def:subdiv_partition}.}
\end{proposition}
\begin{proof}
    \robert{The iteration reaches its stopping cycle by Corollary~\ref{cor:pinch_rounds}, so the output exists, and the stopping rule says that the detection~\eqref{eq:pinch_round_detection} catches nothing there. It remains to see that the detection tests enough, that is $\V^{\mathrm{pinch}}(\domseq[\L]^{\mathrm{pinch},c}) \subseteq \V^{\mathrm{cand},c}$ for every $c$. We show this by induction, the case $c = 0$ being the hypothesis on the seed. Let $\vertex{\l}{\coarseIdxone} \in \V^{\mathrm{pinch}}(\domseq[\L]^{\mathrm{pinch},c})$. It cannot have been a pinch of $\domseq[\L]^{\mathrm{pinch},c-1}$, because it would then lie in $\V^{\mathrm{cand},c-1}$ by induction, hence be caught with $\targetlevel^c_\coarseIdxone > \l$, and Corollary~\ref{lemma:pinch_spent} forbids a pinch below the target. So the pinch is new, and Definition~\ref{def:candidate_collection} puts $\vertex{\l}{\coarseIdxone}$ into $\collectV(\domseq[\L]^{\mathrm{pinch},c-1}, \simplexsubset{2,c}{})$. The set subtracted in~\eqref{eq:pinch_candidates} carries no pinch of $\domseq[\L]^{\mathrm{pinch},c}$, again by Corollary~\ref{lemma:pinch_spent}, so $\vertex{\l}{\coarseIdxone} \in \V^{\mathrm{cand},c}$. With the invariant, an empty detection on the stopping cycle reads $\V^{\mathrm{pinch}}(\domseq[\L]^{\mathrm{pinch}}) = \varnothing$.}
\end{proof}

\begin{remark}[Why the bound is not local]
    \label{rem:pinch_rounds_bound}
The right-hand side of~\eqref{eq:pinch_rounds} is the size of the input domain sequence. The corollary therefore settles only eventual termination, and better upper bounds remain an open question in this work. This remark explains where a sharper bound would have to come from and why it is difficult to achieve.

The iteration is local from one cycle to the next because it only advances the frontier of candidates by one ring of faces, so the refinement never jumps to an unrelated part of the mesh. The iteration bound factors~\eqref{eq:pinch_rounds} roughly into two contributions: the number of classes ever reached by the expanding frontier and the number of times one class is caught within that frontier.
...
This substep detects confined faces with Definition~\ref{def:confined_face} and repairs them with the face quadrisection $\quadriF$. We write
\begin{equation}
...
\begin{lemma}[Confinement padding confines only its own children]
    ...
    Let $\domseq[\L] \in \alldomseqs_\L$, let $\F^\star \subseteq \F^{\mathrm{confined}}(\domseq[\L])$ with slices $\F^\star_\l$, and let $\domseq[\L]' \coloneq \quadriF(\domseq[\L], \F^\star)$ as in~\eqref{eq:confined_faces_resolved_subdiv_partition}. Then every leaf face of $\domseq[\L]'$ that is confined in $\domseq[\L]'$ and was not confined in $\domseq[\L]$ is a child of a face of $\F^\star$. The first term of~\eqref{eq:candidate_collection_faces} may therefore be dropped,
    \begin{equation}
        \label{eq:confined_children}
        \bigsqcup\limits_{\l = \lzero}^{\L-1} \Aop{}{\l}{\l+1} \big( \F^\star_\l \big) \; \subseteq \; \collectF \big( \domseq[\L], \, \F^\star \big) ,
    \end{equation}
    and the smaller set is still a sufficient candidate set.
\end{lemma}
\begin{proof}
    \robert{Let $\face{\l}{\fineIdxone}$ be a leaf face of $\domseq[\L]$ that is unconfined in $\domseq[\L]$ and is still a leaf face of $\domseq[\L]'$, and let $\vertex{\l}{\coarseIdxone}$ be one of its vertices that is refinement-exterior on level $\l$. Then $\vertex{\l}{\coarseIdxone}$ is a vertex of no face of the slice $\F^\star_\l$. Such a face is confined and therefore has no refinement-exterior vertex on level $\l$ by Definition~\ref{def:confined_face}, whereas the fan of $\vertex{\l}{\coarseIdxone}$ is empty there. Lemma~\ref{lemma:pad_locality} therefore leaves $\refFan{\l}(\vertex{\l}{\coarseIdxone})$ unchanged, the vertex stays refinement-exterior and $\face{\l}{\fineIdxone}$ stays unconfined. The only faces of $\domseq[\L]'$ that are not faces of $\domseq[\L]$ are the children of $\F^\star$.}
\end{proof}

\robert{Both substeps narrow the collection of Definition~\ref{def:candidate_collection}, each by its own argument. The pinch resolution subtracts what Corollary~\ref{lemma:pinch_spent} has spent. This substep drops the one-ring term. Let $\F^{\mathrm{seed}} \subseteq \leafMesh(\domseq[\L]^{\mathrm{pinch}})$ be a \emph{seed}.}

\begin{iteration}[Confinement padding]
\label{it:confined_iteration}
    The confinement padding step is initialised with the domain sequence $\domseq[\L]^{\mathrm{confined},0} \coloneq \domseq[\L]^{\mathrm{pinch}}$ and candidate set $\F^{\mathrm{cand},0} \coloneq \F^{\mathrm{seed}}$. It then iterates the following \emph{confinement cycle} for $c \in \{1, \, \ldots, \, q^{\mathrm{confined}} \}$:
    \begin{enumerate}[topsep=3pt, itemsep=-2pt]
    \item Detect confined faces for the candidate set $\F^{\mathrm{cand},c-1}$ of the previous cycle on the domain sequence $\domseq[\L]^{\mathrm{confined},c-1}$ to obtain
    \begin{equation}
        \label{eq:confined_cycle_detection}
        \F^{\mathrm{confined},c} \; \coloneq \; \big\{ \face{\l}{\fineIdxone} \in \F^{\mathrm{cand},c-1} \; \colon \;\; \face{\l}{\fineIdxone} \text{\; is confined on } \domseq[\L]^{\mathrm{confined},c-1} \big\}.
    \end{equation}
    \vspace{-7mm}
    \item Repair the detected faces by refining every one of them by one level,
    \begin{equation}
        \label{eq:confined_cycle_repair}
        \domseq[\L]^{\mathrm{confined},c} \; \coloneq \; \quadriF \big( \domseq[\L]^{\mathrm{confined},c-1}, \, \F^{\mathrm{confined},c} \big).
    \end{equation}
    \vspace{-7mm}
    ...
    \begin{equation}
        ...
        \F^{\mathrm{cand},c} \; \coloneq \bigsqcup\limits_{\l = \lzero}^{\L-1} \Aop{}{\l}{\l+1} \big( \F^{\mathrm{confined},c}_\l \big),
    \end{equation}
\end{enumerate}
We repeat steps $1$ to $3$ until the confinement detection in~\eqref{eq:confined_cycle_detection} detects no more confined faces. We then set $\domseq[\L]^{\mathrm{confined}} \coloneq \domseq[\L]^{\mathrm{confined},c}$ and write $q^{\mathrm{confined}} \coloneq c \! - \! 1$ for the number of cycles until the fixed point was reached.
\end{iteration}

The iteration stops when the confinement detection catches nothing, i.e. $\F^{\mathrm{confined},c} = \varnothing$. That is an empty marked family, so~\eqref{eq:confined_cycle_repair} adds no refinement and~\eqref{eq:confined_candidates} leaves $\F^{\mathrm{cand},c} = \varnothing$. It is also the fixed point $\domseq[\L]^{\mathrm{confined},c} = \domseq[\L]^{\mathrm{confined},c-1}$ of the iteration.

The seed $\F^{\mathrm{seed}}$ may always be taken to be all leaf faces $\leafMesh(\domseq[\L]^{\mathrm{pinch}})$ of the input domain sequence. A smaller seed can be used whenever that sequence is already known to carry no confined face outside a given set, which is the case in every pass of the overarching padding loop in Section~\ref{subsec:termination}.

\begin{corollary}[The confinement iteration advances one level per cycle]
    \label{cor:confinement_cycles}
    Let $\l_c$ denote the coarsest level carrying a face of $\F^{\mathrm{confined},c}$ at cycle $c$. Then $\l_{c+1} \geq \l_c \! + \! 1$, and the number $q^{\mathrm{confined}}$ of productive cycles of Iteration~\ref{it:confined_iteration} satisfies
    \begin{equation}
...
\end{corollary}
\begin{proof}
    By Lemma~\ref{lemma:confined_children} the set $\F^{\mathrm{confined},c+1}$ is contained in the candidate set $\F^{\mathrm{cand},c}$ of~\eqref{eq:confined_candidates}, whose faces are children of faces of $\F^{\mathrm{confined},c}$ and hence lie on levels $\lone \geq \l_c \! + \! 1$. Equality can fail, because no child of a face on level $\l_c$ has to be confined. A face whose neighbourhood is refined exactly one level beyond $\l_c$ leaves four children with a refinement-exterior vertex on level $\l_c \! + \! 1$ and drops out, and $\l_{c+1}$ is then read off whichever face of $\F^{\mathrm{confined},c}$ is buried deeper. No leaf face on level $\L$ is ever confined, since every vertex is refinement-exterior on level $\L$ by Definition~\ref{def:refinement_fan}. Hence $\l_c \leq \L \! - \! 1$ for every productive cycle, which gives~\eqref{eq:confinement_cycles}.
\end{proof}
...
\begin{proof}
    ...
    \begin{equation*}
        \F^{\mathrm{confined}} \big( \domseq[\L]^{\mathrm{confined},c} \big) \; \subseteq \; \F^{\mathrm{cand},c} .
    \end{equation*} A face already confined in $\domseq[\L]^{\mathrm{confined},c-1}$ lies in $\F^{\mathrm{cand},c-1}$ by induction, so~\eqref{eq:confined_cycle_repair} refines it and it is no longer a leaf face, which replaces the appeal to Corollary~\ref{lemma:pinch_spent}. A newly confined face is a child of $\F^{\mathrm{confined},c}$ by Lemma~\ref{lemma:confined_children} and hence lies in $\F^{\mathrm{cand},c}$.}
\end{proof}

% 2026-09-02: superseded by the frontier-direction paragraph of the Summary,
% which makes the same contrast for both substeps at once. Written as a plain
% paragraph rather than a remark environment so it consumes no theorem number.
% Kept for cherry-picking.
\tobecut{\textbf{Superseded remark, why this bound is a level count.} The bound~\eqref{eq:confinement_cycles} is in the number of levels and not in the size of the mesh. That is the contrast with Remark~\ref{rem:pinch_rounds_bound} for pinch resolution. Lemma~\ref{lemma:confined_children} confines the next cycle to the children of this one, so the coarsest defect level strictly increases and the iteration cannot chain sideways.}
...
\robert{The two cycle counts $q^{\mathrm{pinch}}$ of Corollary~\ref{cor:pinch_rounds} and $q^{\mathrm{confined}}$ of Corollary~\ref{cor:confinement_cycles} are the counts of the two inner iterations. They are not part of the count $q$ of productive $\textsc{Pad}$ passes of Section~\ref{subsec:termination}, which counts the passes driven by the links between the substeps alone.}

\subsubsection{Grading leaf stars}
\label{subsubsec:grading}

% 2026-09-02: the conditioning justification is not supported. Measured on the
% L=4 run of Section~\ref{subsubsec:cl_performance}, kappa(P^-1 K) is flat with
% and without the grading. Replaced by the coarse-mass rationale, which is
% measured. Kept for cherry-picking.
\tobecut{The grading is supposed to smooth out transitions across vertex classes $\vertexClass{\coarseIdxone}$ of $\domseq^{\mathrm{confined}}$ whose leaf stars still have large level spans, see Definition~\ref{def:level_range}. Unlike the pinch resolution and the confinement padding, the grading step does not restore exactness or linear independence but is intended to improve the conditioning of the matrix-vector system and to avoid numerical artifacts in the transition areas. Thus, grading is an optional padding step. It is the last of the three substeps, and it is a single sweep and not an iteration.}

\robert{The grading closes the level span of Definition~\ref{def:level_range} at the vertex classes of $\domseq^{\mathrm{confined}}$ that carry a wide one. It raises $\lminIdx{\coarseIdxone}$ towards $\lmaxIdx{\coarseIdxone}$ and leaves the fine end alone, so it refines at the coarse end of a leaf star and nowhere else.}

\robert{Grading is optional for admissibility. No clause of Definition~\ref{def:subdiv_partition} mentions the level span, and the two substeps above discharge all three clauses without it. What it is for is the coarse end of the mesh. The marking of Section~\ref{subsubsec:refine_vertex} ranks vertex classes by the area density~\eqref{eq:vertex_residual}, so a coarse face away from the refined region ranks last and is never selected. Nothing else in the loop raises such a face either. Estimator mass sitting on it therefore stays there, while the bulk criterion~\eqref{eq:doerfler_selection} goes on counting it. The grading is the one step that moves that mass up a level.}

\robert{Retuning the ranking is not a substitute, because the two act on different scales. A marked class is homogenised to $\targetlevel_\coarseIdxone = \min(\lmaxIdx{\coarseIdxone} \! + \! 1, \L)$, which advances a whole star by one level, whereas the grading refines only the coarse end of one. Section~\ref{subsubsec:cl_performance} measures both.}

\robert{The grading is the last of the three substeps, and it is a single sweep and not an iteration.}
...
\robert{Unlike~\eqref{eq:pinch_detection_target} this map needs no $\lminIdx{\coarseIdxone}$ floor to be total. A caught class has $\levelSpan{\coarseIdxone} > g$ and therefore $\lminIdx{\coarseIdxone} = \lmaxIdx{\coarseIdxone} - \levelSpan{\coarseIdxone} < \lmaxIdx{\coarseIdxone} - g = \targetlevel_\coarseIdxone$, so the target already sits strictly above the coarse end of the leaf star and Corollary~\ref{cor:graded_quadrisection_composition} never reads the operator as the identity at a caught class.}
...
\robert{The extra level is spent only under the condition recorded after Corollary~\ref{cor:graded_quadrisection_leafstar}, which the target~\eqref{eq:grading_detection_target} reads as a condition on two leaf stars. The neighbour has to be more than $g$ levels finer than $\vertexClass{\coarseIdxone}$, so that $\lmaxIdx{\coarseIdxtwo} > \lmaxIdx{\coarseIdxone} + g$, and it has to be still touching $\vertexClass{\coarseIdxone}$ at the fine end of its leaf star, so that $\lmeetIdx{\coarseIdxone}{\coarseIdxtwo} = \lmaxIdx{\coarseIdxone}$. That section shows the condition is attainable and that the lift stays at one level however far past $\lmaxIdx{\coarseIdxone}$ the neighbour is graded.}
...
\label{sw:grading_sweep}
    ...
    \end{equation}
    \vspace{-7mm}
    \item \robert{Collect the candidates for the next padding cycle from the marked family $\simplexsubset{2}{}$ that~\eqref{eq:graded_marked_family} builds from $\gradeClasses$ and $\targetlevel$,}
    \begin{equation}
    ...
    \end{equation}
\end{enumerate}
\robert{The two sets of step $3$ seed the pinch resolution and the confinement padding of the next padding cycle. The sweep is not repeated.}
\end{sweep}

\robert{The vertex set of~\eqref{eq:grading_sweep_candidates} is enough for the pinch side, because Remark~\ref{rem:new_vertices} retires the vertices the quadrisection creates there. The face set is needed in full, both terms of~\eqref{eq:candidate_collection_faces}. The slice refined here is a class star and not a set of confined faces, so Lemma~\ref{lemma:confined_children} does not apply and the children have to be carried along with the one-ring, exactly as in~\eqref{eq:seed_confined}. The grading collects nothing for itself.}

\robert{The seed $\vertexClasses^{\mathrm{seed}}$ is all classes $\vertexClasses(\domseq[\L]^{\mathrm{confined}})$ of the input domain sequence, and unlike the other two substeps the grading does not narrow it. The remark after Definition~\ref{def:candidate_collection} says why. A class the sweep has to test need not be one that any earlier slice carries, since a newborn midpoint can arrive with a large span of its own. The seed is kept as an argument only so that Proposition~\ref{prop:grading_span} states the hypothesis it actually uses.}
...
\guidance{Nothing in Section~\ref{sec:numerical_experiments} separates what the grading costs from what admissibility costs. Table~\ref{tab:padding_interface_L4} reports the outer count $q$ and a single \emph{padded} face column that lumps all three substeps together. Splitting that column and running $\L = 4$ at $g = \infty$ against $g = 2$ is what would decide whether the grading survives into a paper at all.}

% SUPERSEDED (2026-09-01): the per-vertex enumeration and the claim that one
% sweep closes the span to g. Replaced by Sweep~\ref{sw:grading_sweep} and
% Proposition~\ref{prop:grading_span}. Labels suffixed _old. Kept for cherry-picking.
\tobecut{The grading is supposed to smooth out transitions across vertices $\vertex{\l}{\coarseIdxtwo}$ of $\domseq^{\mathrm{confined}}$ whose leaf stars still have large level spans, see Definition~\ref{def:level_range}. Unlike the pinch resolution and the confinement padding, the grading step does not restore exactness or linear independence but is intended to improve the conditioning of the matrix-vector system and to avoid numerical artifacts in the transition areas. Thus, grading is an optional padding step. \robert{It is the last of the three substeps, and it is run as a single sweep rather than as an iteration.}

The predicate to identify the vertices $(\vertex{\l}{\coarseIdxtwo})$ to grade is the level span $\mu_\coarseIdxtwo \coloneq \lmax{}_{,\coarseIdxtwo} - \lmin{}_{,\coarseIdxtwo}$ of its leaf star $\FVleaf{}(\vertexClass{\coarseIdxtwo})$. The detection algorithm fixes a maximum level span $g \in \mathbb{N}$ and assembles the set
\begin{equation}
    \label{eq:grading_set_old}
    \V^{\mathrm{grading}}(\domseq[\L]) \coloneq \big\{ \vertex{\l}{\coarseIdxtwo} \in \V(\domseq[\L]) \; \colon \; \mu_\coarseIdxtwo > g \big\}
\end{equation}
...
\begin{equation}
    \label{eq:grading_subdiv_partition_old}
    \domseq^p = \quadriV \big( \domseq^{p-1}, \, \{ \vertexClass{\coarseIdxtwo_p} \}, \, \targetlevel \big) \quad \text{for all} \quad (\vertex{\l}{\coarseIdxtwo})_p \in \V^{\mathrm{grading}}(\domseq[\L]^\mathrm{confined}),
\end{equation}
where $p \in \{1, \, \dots, \, \vert \V^{\mathrm{grading}}(\domseq[\L]^\mathrm{confined}) \vert \}$ is an enumeration of the vertices to be graded and $\domseq[\L]^0 = \domseq[\L]^{\mathrm{confined}}$. Note the additional $-g$ in the target level map $\targetlevel_\coarseIdxtwo = \lmax{}_{,\coarseIdxtwo} - g$. \robert{Applied to $\vertexClass{\coarseIdxtwo}$ on its own, Corollary~\ref{cor:graded_quadrisection_leafstar} raises the coarsest level $\lminIdx{\coarseIdxtwo}$ of its leaf star to $\lmaxIdx{\coarseIdxtwo} - g$ and leaves $\lmaxIdx{\coarseIdxtwo}$ where it is, so the level span closes to exactly $g$. The classes are graded as a set, and then the span can come out on either side of $g$ by~\eqref{eq:graded_quadrisection_leafstar_set}, but by at most one level in the fine direction. It is smaller when the grading of a neighbour lifts the coarse end of this leaf star as well. It is larger, and then by exactly one, when a class still adjacent to $\vertexClass{\coarseIdxtwo}$ on level $\lmaxIdx{\coarseIdxtwo}$ is graded past that level, which asks it to carry a finest level more than $g$ above $\lmaxIdx{\coarseIdxtwo}$. So one sweep leaves every class at a level span of at most $g \! + \! 1$.} We use $g = 2$ throughout, which works well in the simulations of Section~\ref{subsec:vector_laplace}.

\robert{The grading is a single sweep per cycle and it comes last, which is what makes it read as an optional tack-on rather than a stage of its own.

The reason is that it never has much to do. It is tempting to picture the loop starting from a domain sequence with wild level spans that the grading flattens before the small repairs run, but that picture is wrong. By Section~\ref{subsec:refine} the \textsc{Refine} step raises $\lmaxIdx{\coarseIdxtwo}$ by exactly one level at each marked class, and the two padding substeps ahead of it raise it at no class at all. A pinch is detected at a level $\l$ whose refinement fan is a non-empty proper subset of the mesh star, so $\lminIdx{\coarseIdxtwo} \leq \l < \lmaxIdx{\coarseIdxtwo}$ by Proposition~\ref{proposition:class_star_levels} and the target $\l \! + \! 1$ does not exceed $\lmaxIdx{\coarseIdxtwo}$. A confined face on level $\l$ has no refinement-exterior vertex there, so $\l < \lmaxIdx{\coarseIdxone}$ at each of its three vertices by the same proposition, and its children on level $\l \! + \! 1$ stay at or below $\lmaxIdx{\coarseIdxone}$ as well. A leaf star arriving at the grading step therefore carries a span at most one level over $g$, never more, and one sweep is enough to return it to $g$.

That is also why the grading needs no inner iteration of its own. It is the outer loop that closes the span, since the grading predicate is one of the three tested on every cycle, so a fixed point of $\textsc{Pad}$ carries level span at most $g$ by construction. What the grading can open on its way is repaired by the next cycle like any other defect.}}
...
\robert{What one padding cycle does not do is leave an admissible domain sequence. Each substep can reopen the predicate of another, so the cycle has to be iterated, which is what the next Section~\ref{subsec:termination} studies.}
...
\end{equation}
\robert{whose first two factors are the iterations of Section~\ref{subsubsec:pinch_resolution} and Section~\ref{subsubsec:confined_faces} and not single sweeps.} Our implementation wraps $\textsc{Pad}$ in a loop that stops once a pass no longer refines any face, and a counter records the number $q$ of \emph{productive} passes,
\begin{equation}
...
\begin{remark}[\robert{Seeding one substep from another}]
    \label{rem:substep_seeding}
    \robert{The chain of the next remark travels through a set this remark makes explicit. No substep has to start from the whole domain sequence. The initial candidate sets of the two substeps that restore admissibility are stated as $\V(\domseq[\L]^{\mathrm{refine}})$ and $\leafMesh(\domseq[\L]^{\mathrm{pinch}})$ in Section~\ref{subsec:padding} for readability, and the results already proved shrink both. Three of them compose. Each of the two discharges its own predicate, by Proposition~\ref{prop:pinch_discharged} and Proposition~\ref{prop:confined_discharged}. Lemma~\ref{lemma:pad_locality} says where a quadrisection can move a predicate. Remark~\ref{rem:new_vertices} settles the vertices a quadrisection creates, and the remark after it settles the faces.

    Write $\simplexsubset{2}{\l}$ for the union over the rounds of a substep of the level-$\l$ slices it refined. Two handoffs follow.

    From confinement to pinch. The sequence entering the confinement iteration carries no pinch on any level. So the pinch iteration of the next cycle may start from the vertices of what the confinement padding refined, together with what the grading refined if it was run,
    \begin{equation}
        \label{eq:seed_pinch}
        \V^{\mathrm{cand},0} \; = \bigsqcup\limits_{\l = \lzero}^{\L-1} \V_\l \big( \simplexsubset{2}{\l} \big).
    \end{equation}
    % 2026-09-02: made conditional on the grading being run, since the grading is
    % now stated as optional for admissibility. Kept for cherry-picking.
    \tobecut{The grading has to be counted in, because it is a graded vertex quadrisection and runs last, so it can reopen all three predicates.} \robert{The grading contributes a slice only when it is run. When it is, that slice has to be counted in, because the grading is a graded vertex quadrisection and comes last, so it can reopen all three predicates. When the grading is switched off, $\simplexsubset{2}{}$ carries the confinement slices alone and the seed is correspondingly smaller.} Note also that these are the faces \emph{refined} and not the faces added. A predicate moves on level $\l$ at the vertices of the level-$\l$ slice, whereas the children lie on level $\l\!+\!1$ and the vertices born with them are settled by Remark~\ref{rem:new_vertices}.

    From pinch to confinement. The confinement iteration of a cycle may start from the one-ring of the slices refined ahead of it together with their children,
    \begin{equation}
        \label{eq:seed_confined}
        \F^{\mathrm{cand},0} \; = \; \collectF \big( \domseq[\L], \, \simplexsubset{2}{} \big) .
    \end{equation}
    % 2026-09-02: made conditional, as in eq:seed_pinch. Kept for cherry-picking.
    \tobecut{Here $\simplexsubset{2}{}$ carries the pinch slices of this cycle \emph{and} the grading slice of the previous one. The grading has to be counted in for the same reason it is counted into~\eqref{eq:seed_pinch}.} \robert{Here $\simplexsubset{2}{}$ carries the pinch slices of this cycle, and the grading slice of the previous one whenever the grading was run. It is counted in for the same reason it is counted into~\eqref{eq:seed_pinch}.} It runs last, and it can confine a leaf face. Take a leaf face on level $\l$ that is unconfined only through one vertex $\vertex{\l}{\coarseIdxone}$. If a graded class star holds another face at $\vertex{\l}{\coarseIdxone}$ but not this one, then the sweep leaves the face a leaf face and makes $\vertex{\l}{\coarseIdxone}$ refinement-interior, so the face is confined in the output. \tobecut{Only the \textsc{Refine} step of the first cycle may be read without a grading term ahead of it, since its input is admissible.} \robert{With the grading running, the only cycle whose seed carries no grading term ahead of it is the first, since the input of the \textsc{Refine} step is admissible.}

    % 2026-09-02: re-derives what Remark~\ref{rem:padding_interactions} already
    % settles for the same term. Kept for cherry-picking.
    \tobecut{The second term of~\eqref{eq:candidate_collection_faces} is not optional here. Clause~$(ii)$ of Lemma~\ref{lemma:pad_locality} speaks only about faces that were already leaf faces, and a child of a repaired class star can be confined by the same burial that Corollary~\ref{cor:confinement_cycles} counts. Lemma~\ref{lemma:confined_children} does not apply, because the slice refined here is a class star and not a set of confined faces.}

    \robert{Both terms of~\eqref{eq:candidate_collection_faces} are needed here, since the slice refined ahead of this iteration is a class star and not a set of confined faces, so Lemma~\ref{lemma:confined_children} does not apply.}

    Both seeds close the loop back to \textsc{Refine}. Its input is admissible, so the first cycle may be seeded by~\eqref{eq:seed_pinch} and~\eqref{eq:seed_confined} read on the marked family of the \textsc{Refine} step. The two substeps that restore admissibility then never scan the domain sequence at all, and touch only what the step before them has refined. The grading is left out of this. It is optional, its predicate is read off a leaf star that an earlier slice need not carry, and a scan of the classes is cheap next to a repair.}
\end{remark}

\guidance{The implementation does not do this. All three substeps restart from the full scan, which is what Section~\ref{subsec:padding} describes and what the reported counts come from. The saving is real but not on the critical path, since the scan is linear in the mesh and the solve dominates. Measured on the valence-$10$ domain sequence of the candidate-set figure, once that figure lands and this becomes a \texttt{\textbackslash ref}, the seed~\eqref{eq:seed_pinch} carries $16$ vertices against the $629$ that the full initial set carries.}

...
\end{lemma}
\begin{proof}
    At a pinch vertex $\vertex{\l}{\coarseIdxone}$, two distinct refinement arcs of the refinement fan at level $\l$ are separated by at least one face by Definition~\ref{def:pinch_vertex}. That face is not refined beyond level $\l$, so its children incident to $\vertex{\l}{\coarseIdxone}$ are not leaf faces, and the last factor $\quadriVat{\l}$ of $\quadriVup{\l+1}$ quadrisects at least one of them. The same argument covers a boundary pinch, whose unanchored arc is separated from both extremes of the linear refinement fan by such a face. An inadmissible face is detected on the leaf faces only, so $\quadriF$ is not the identity on it. A vertex of level span greater than $2$ carries leaf faces below $\targetlevel_\coarseIdxone \! - \! 2$, so $\quadriVup{\targetlevel_\coarseIdxone - 2}$ quadrisects at least one of them. Pinches are detected only at levels $\l < \L$ and the grading target never exceeds $\L \! - \! 2$, so no repair refines beyond level $\L$.
\end{proof}
...
\label{rem:padding_order}
    Resolving a pinch vertex can make an admissible face inadmissible (e.g.\ Figure~\ref{fig:padding_chain}), the admissibility padding can in turn create a pinch, and the grading, being a vertex quadrisection, can create both. A single pass therefore never suffices in general, whichever order is used, and the order is not what makes $\textsc{Pad}$ terminate. The proof of Lemma~\ref{lemma:pad_productive} uses only that a substep is the identity when its own predicate does not fire, so the conclusion above holds for every ordering of the three substeps. 

    Pinch resolution implies more additional refinement than admissibility padding and grading, and thus changes $\domseq[\L]$ the most. For that reason, we chose to run it first, so that the defects it produces can be repaired downstream by the other two steps.
\end{remark}

What keeps $q$ from being bounded by the irregular vertices of the control mesh $\T_\lzero$ is a
mechanism rather than an accident.

\begin{remark}[Chaining of padding passes]
    \label{rem:padding_chain}
    What drives $q$ up is a chain of alternating pinch resolutions that create inadmissible faces and admissibility padding that creates pinches, see for example Figure~\ref{fig:padding_chain}.

    Consider a pinch vertex that is regular, i.e. is has valence $6$, and let $\L=1$ so that the $\textsc{Refine}$ step can only refine vertex stars. That implies that the pinch has two refinement arcs of 2 formerly coarse faces, while the gaps cover only one coarse face. This configuration will be resolved by the admissibility padding step. If the pinch vertex was irregular, for example with valence $8$ as in Figure~\ref{fig:padding_chain}, then both of its refinement arcs and the gaps can cover two coarse faces each. In that case, admissibility padding does not resolve the pinch and we need to wait for the next iteration of the pinch resolution.

    This is exactly how the chain in Figure~\ref{fig:padding_chain} works: We start from a domain sequence with a pinch at an irregular vertex. The first iteration fist resolves that pinch by refining its gap faces. The newly refined region now touches the next irregular vertex, and because of the particular location of the third vertex star, one face becomes inadmissible and needs refinement. Crucially, the other faces touching this newly created pinch vertex are admissible because they belong to refinement arcs with two or more faces. If that was not the case, the first admissibility pass would refine them as well and the pinch would be resolved before the second iteration. This effect repeats as long as there are irregular vertices to propagate along.

    With $\L = 1$, arcs of a single face and hence gaps of two or more faces arise only around irregular vertices, so the padding chains only where $\T_\lzero$ carries irregular vertices close enough to each other, and only where $\textsc{Refine}$ produces the two-arc configuration on top of that. The maximum length of such a chain is a property of the initial mesh $\T_\lzero$ and could bound $q$ for $\L=1$.
\end{remark}
...
\begin{figure}[tb]
    ...
    \chainpanel{images/padding_chain/chain_input.pdf}
        {Input domain sequence $\domseq[\L]$ with pinch produced by the \textsc{Refine} step.}
        {fig:padding_chain_input}%
    \hfill%
    \chainpanel{images/padding_chain/chain_pass0.pdf}
        {Pass~1 resolves first pinch, pads one face, and creates a second pinch.}
    ...
    \caption{A control mesh with three interior vertices of valence $8$ in a row on which $\textsc{Pad}$ needs one productive pass per vertex. The three valence-$8$ vertices are drawn in colour, and the one resolved in a pass is emphasized with a ring. Each pass resolves exactly one pinch vertex and pads a single face that has become inadmissible after the pinch resolution. That padded face creates the next pinch along the vertex chain, so the refinement propagates down one vertex per pass. A fourth, unproductive pass confirms the fixed point.}
    \label{fig:padding_chain}
\end{figure}

\begin{remark}[Chaining at regular vertices]
\label{rem:regular_chain}
Once we allow $\L \geq 2$, even regular vertices can become pinches with two gaps of two coarse faces. Refining the interface of an existing fine patch can refine a single coarse face and create an arc of one face, cf. the right-hand pair of Figure~\ref{fig:refine_interface}. The pinch survives the admissibility padding pass and needs a second pinch resolution pass. Thus, the number of irregular vertices of $\T_\lzero$ therefore does not bound $q$, and we can observe a second productive pass on meshes that carry no irregular vertex at all.
\end{remark}

We do not know whether a whole chain can run through regular vertices. The reason for caution is that the interface refinement supplies exactly the ingredient a chain needs, an arc of a single face, and supplies it at a vertex of any valence. The reason it may nonetheless be blocked is that interface refinement never supplies that arc alone: the marked class that lifts the coarse face also refines the faces around it, so every single-face arc it creates carries a bulk of refinement beside it. A surviving pinch needs two such arcs, and hence two such bulks, and the bulks may always come to lie close enough to the pinch for the admissibility padding of the same pass to reach the configuration and close it. In that case, there is no corridor for a chain to run along, and the mechanism costs one pass wherever it fires. We have neither constructed a chain through regular vertices nor excluded one.

\begin{problem}[Bounding the number of padding passes]
    \label{prob:padding_passes}
    Bound the number $q$ of productive $\textsc{Pad}$ passes by a quantity depending only on the control mesh $\T_\lzero$. Confine the faces refined by $\textsc{Pad}$ to a neighbourhood of the defects of $\domseq[\L]$ of bounded size.
\end{problem}

The second requirement rules out the degenerate answer $\textsc{Pad}(\domseq[\L]) = \T_\L$, which stabilises in one pass but defeats the purpose of the adaptive algorithm.

We reiterate how rare padding chains are. The configurations with $q > 1$ above were searched for deliberately, in order to isolate the mechanism of Figure~\ref{fig:padding_chain}, whereas every run of the test cases of Section~\ref{sec:numerical_experiments} stabilises after a single productive pass and pads a small fraction of the faces refined by the marking, see Table~\ref{tab:padding_interface_L4}.
